...
// FakeWorker` compiles the impl block without help, but `.start(..)` and `.read_one(..)` below
// are METHOD-CALL syntax, and Rust resolves those only against traits already IN SCOPE --
// measured, not assumed: without the import the compiler answered `E0599` on
