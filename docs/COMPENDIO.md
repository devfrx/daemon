...
> (777 KB in byte LF il 2026-08-25, e possono solo crescere — la spec da sola ne fa 277), e
> l'idea è già qui.
...
| 59 | ⛔ **Un ADR può essere falsificato da un ADR FRATELLO scritto lo stesso giorno, e nessuno dei due se ne accorge perché entrambi sono coerenti con sé stessi.** Misurato il 2026-08-11: [ADR-0026](adr/0026-linguaggio-del-core.md) elenca fra le proprie `Positive` *«esiste un runtime deterministico di ecosistema — `madsim` 0.2.34 … quindi il simulatore non va scritto da zero»*, e [ADR-0031](adr/0031-dipendenze-del-kernel-parte-del-confine.md) — **`Date: 2026-08-06`, la stessa** — misura che una crate `no_std` che dipende da `madsim` **compila** con **55 crate** nel grafo e conclude *«55 crate di superficie non verificabile contro un esecutore che nel prototipo è ~30 righe»*. Il codice di oggi dà ragione a 0031: `simulator` ha **una** dipendenza (`kernel`), è scritto **a mano**, e `madsim` non compare né in `Cargo.lock` né in `crates/`. ⚠️ **Quante righe non è scritto qui — richiamo del 2026-08-25:** questa riga diceva **512**, misurate il 2026-08-11 dal commit che la scrisse, e il 2026-08-25 erano **519**, mosse da `b4f1f03`; la cifra viveva in **quattro** documenti ed è **tolta** e non riallineata, perché il conto lo dà `find crates/simulator/src -name '*.rs' \| xargs wc -l` e l'argomento — la contraddizione fra i due ADR — regge identico a qualunque valore. 📌 È il **#31** su una cifra che nessuno rimisurava perché **sosteneva una tesi giusta**. ⛔ **Non è il #57**: là la decisione è **anteriore** a ciò che nomina e la smentita arriva dopo, dal codice; qui la smentita è **contemporanea** e sta in un altro file della stessa cartella. ⚠️ **Nessuno dei due ADR nomina l'altro**, quindi la contraddizione non è visibile da nessuno dei due lati. 📌 **La domanda che lo coglie, e nessuna delle quattro del pre-controllo la fa:** *questo ADR è stato letto contro i suoi **fratelli**, non solo contro il codice?* Le quattro domande guardano tutte il compito contro il codice |
...
| il testo integrale dei **gotcha** e delle **misure**, con i numeri | [`HANDOFF.md`](HANDOFF.md) — ⚠️ **a sezioni** | 277 KB |
...
| **dove vive ogni controllo** della porta, riga per riga sul catalogo §7.4, e cosa **non** è coperto | [`porta-di-qualita.md`](porta-di-qualita.md) | 416 KB |
...
| l'indice di ADR e diagrammi | [`README.md`](README.md) | 21 KB |
...
> | | |
> |---|---|
> | **cresciuti** | questo file `548 → 564` — la §6 riscritta in tre punti, la riga della §9
...
> questo file `562 → 579` · con l'audit `594 → 611` · [`HANDOFF.md`](HANDOFF.md) più la spec
