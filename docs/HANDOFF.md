...
⏭️ **Proseguire l'esecuzione del piano del Traguardo 4, dal Task 6** — subagent-driven, un
compito per volta con revisione fra uno e l'altro. ✅ **Cinque compiti su dieci eseguiti il
2026-08-11**, `GATE GREEN` a ciascuno: la **Parte 1 è completa** — il giornale che cade, lo
scenario giornalato con `C7a`, **`C7b`**, la campagna breve col numero di semi **misurato** — e il
**Task 5 ha aperto la Parte 2**, dove il soggetto non è più la riconciliazione del kernel ma la
**coerenza di `redb` dopo il crash**.
...
| ✅ **come si chiude il #51** | con `Durability::None` `redb` **non chiama `sync_data`**: un backend che **conta** le chiamate lo rende osservabile, e il conteggio dei punti scattati è **l'oracolo di non-vacuità** della campagna |
