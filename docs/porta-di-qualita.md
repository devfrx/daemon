...
poteva chiudere si chiude.** `V3` — *«una seconda policy attiva: il valore consegnato ne porta
una sola»* — passa da **scoperta** a **coperta**: `VramPolicy` è un enum e `Arbiter::new` lo
prende **per posizione**, quindi due policy sono un errore di **arità**. Blocco C: **da
quattordici a QUINDICI righe su diciannove**, ricontate direttamente su §7.4.1 blocco C e non
da questa frase. Il denominatore non si muove. ⛔ **Il blocco B resta a DUE righe su cinque:**
...
questione di **orchestrazione**, che è del **Task 10** — la radice di composizione. ⚠️ **E
interagisce direttamente con la voce delle due quote permanenti qui sopra**, che al Task 10
...
#### Le due policy VRAM — Traguardo 5, Task 8, e `V3` si chiude
...
`VramPolicy::Remote(RemotePolicy)` fresco invece di quello conservato, quella sonda è **l'unica
rossa dell'intero workspace** (riga **9**); senza quelle due righe la mutazione sarebbe stata un
**mutante vivo**.

⛔ **`V3` SI CHIUDE, E IL CASO NEGATIVO NON È QUELLO CHE IL PIANO DETTAVA.** Il piano scriveva
...
⛔ **`Arbiter::new` AVEVA CINQUE SITI IN QUATTRO FILE, E TRE ERANO CASI `compile_fail` CON
...
⛔ **LA CAMPAGNA DI MUTAZIONE — TREDICI RIGHE, e le ultime due esistono per una misura di
isolamento e non per uccidere.** Ogni mutazione: entrata provata col conteggio delle occorrenze,
...
| # | Mutazione, sul codice di produzione | Sonda attesa morta | Misurato (banco di cinque · `arbiter_admission` venti · `lib` tredici) |
|---|---|---|---|
| **1** | `RemotePolicy::may_make_room` restituisce `true` | `the_remote_policy_does_not_make_room_it_queues` e `under_the_remote_policy_…` | ✅ **rosse tutte e due — 3 passate, 2 fallite.** La prima muore sull'asserzione di `revoking()`. ⚠️ **`arbiter_admission` 20/0 e `lib` 13/0**: nessuna sonda preesistente se ne accorge |
| **2** | `LocalPolicy::may_make_room` restituisce `false` | `the_local_policy_asks_the_lower_lanes_back` | ⛔ **ne uccide DUE e non una — 3 passate, 2 fallite:** muore anche `under_the_local_policy_the_queued_request_is_served_past_the_grace`, che il piano non nomina. Divergenza `E92` |
| **3a** | la domanda tolta da `admit`, **nessuno** fa spazio (il blocco `if` reso irraggiungibile) | come la **2** | ✅ **le due sonde LOCAL — 3 passate, 2 fallite.** La direzione *«scatta dove deve»* |
| **3b** | la domanda tolta da `admit`, **tutti** fanno spazio (la condizione resa sempre vera) | — | ✅ **le due sonde REMOTE — 3 passate, 2 fallite.** La direzione *«non scatta dove non deve»*, che è la metà che si dimentica. ⚠️ **E `arbiter_admission` resta 20/0 e `lib` 13/0** anche qui: il ramo nuovo non è tenuto da niente fuori da questo banco (`E96`) |
| **4** | ⚠️ **il separatore che il PIANO indica**: `promote` percorre le corsie al contrario | `under_the_remote_policy_…` rossa, `the_remote_policy_…` verde | ⛔ **NESSUNA delle cinque muore — 5 passate, 0 fallite.** Con una sola richiesta in attesa l'ordine fra corsie non decide niente. La riga uccide una sonda del Task 6 in `arbiter_admission` (19/1) e **non separa** la coppia della **1**. Divergenza `E92` |
| **5** | il controllo di capienza dentro `promote` tolto: promuove senza stanza | — | ✅ **`under_the_remote_policy_the_same_clock_advance_serves_nobody`, e SOLA NEL BANCO — 4 passate, 1 fallita**, sull'asserzione `promoted.is_empty()`. È il separatore che la **4** doveva essere. Fuori dal banco: `arbiter_admission` 16/4 |
| **6** | `promote` non promuove niente (il controllo di capienza sempre vero) | — | ✅ **`under_the_local_policy_the_queued_request_is_served_past_the_grace`, e SOLA NEL BANCO — 4 passate, 1 fallita.** Separa la coppia che la **2** uccide insieme. Fuori dal banco: `arbiter_admission` 14/6 |
| **7** | `revoking()` risponde sempre **0** | — | ✅ **`the_local_policy_asks_the_lower_lanes_back`, e SOLA NEL BANCO — 4 passate, 1 fallita.** Fuori dal banco: `lib` 7/6 |
| **8** | `revoking()` conta **ogni** concessione nei libri | — | ✅ **`the_remote_policy_does_not_make_room_it_queues`, e SOLA NEL BANCO — 4 passate, 1 fallita.** Fuori dal banco: `lib` 5/8 |
| **9** | `policy()` restituisce un `VramPolicy::Remote(RemotePolicy)` fresco invece di quello conservato | — | ✅ **`each_policy_names_itself`, e SOLA NELL'INTERO WORKSPACE — 4/1, `arbiter_admission` 20/0, `lib` 13/0.** Muore sull'asserzione che legge il nome **LOCAL attraverso l'arbitro** |
| **10** | il dispatch di `VramPolicy::name` risponde `"remote"` per **entrambi** i bracci | — | ✅ **`each_policy_names_itself`, e SOLA NELL'INTERO WORKSPACE — 4/1, 20/0, 13/0.** Muore sull'asserzione che legge il nome **LOCAL dall'enum**: le due metà della sonda stanno su **assi diversi**, ed è per questo che ci sono entrambe |
| **11a** | l'`enqueue` estratto smette di far avanzare il contatore dei biglietti | — | ✅ **`arbiter_admission` 18/2**, banco 5/0. Il corpo estratto è **vivo** ed è tenuto dalle sonde del Task 6 |
| **11b** | l'`enqueue` estratto mette **ogni** richiesta nella corsia `Batch` | — | ✅ **`arbiter_admission` 19/1**, banco 5/0. Idem, sull'altro campo |
| **12** | la spazzata riscuote una concessione `Running` (`_ => true` diventa `_ => false`) | — | ⚠️ **Riga di ISOLAMENTO, non di uccisione.** Banco 3/2 — ma le due morte muoiono sul `panic!("queued")` del `let … else`, **non** sull'asserzione di `allocated()`. `arbiter_admission` 7/13, `lib` 4/9 |
| **13** | `admit` smette di guardare il tetto (sovra-ammissione) | — | ⚠️ **Riga di ISOLAMENTO.** Banco 1/4, e **a piena forza nessuna delle quattro muore sull'asserzione di `allocated()`**: le prime due asserzioni scattano sempre prima. `arbiter_admission` 8/12, `lib` 12/1 |

⛔ **QUALI SONDE HANNO UN UCCISORE SOLO, E CON QUALE PERIMETRO.** **Cinque su cinque** ce l'hanno,
e **nessuna è dominata**: `the_remote_policy_…` sotto la **8**, `the_local_policy_…` sotto la
**7**, `under_the_local_policy_…` sotto la **6**, `under_the_remote_policy_…` sotto la **5**,
`each_policy_names_itself` sotto la **9** e la **10**. ⚠️ **Ma il perimetro non è lo stesso per
tutte, e va detto invece che lasciato intendere:** solo la **9** e la **10** sono sole
nell'**intero workspace**; le altre quattro sono sole **dentro `tests/arbiter_policy.rs`** e
uccidono altrove — le **5** e **6** fra le sonde delle code del Task 6, le **7** e **8** fra
quelle della revoca del Task 7, che leggono `revoking()` anche loro.
...
per prima, in tutte e tredici le righe. ✅ **Misurato per isolamento il 2026-08-20 e non
...
| `crates/kernel/tests/arbiter_policy.rs` | **blocco C** · `V3` — la direzione *«con UNA policy compila»*, e con **entrambe** le policy: il banco costruisce un arbitro per ciascuna, da fuori la crate. ⚠️ **File nuovo del Traguardo 5 Task 8.** ⛔ **La seconda metà della contro-sonda di catalogo — *«e la transizione resta un passo giornalato (§5.4)»* — NON è qui: è del Task 9** | **cinque** test — ricontati **sul binario** |
