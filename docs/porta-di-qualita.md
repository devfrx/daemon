...
| `crates/simulator/tests/gui_death_campaign.rs` (**due** test — contati **sul binario** il 2026-09-02) | ⛔ **la proprietà 3 di §5.7** — la gui muore tenendo una concessione discrezionale e la somma torna alla linea di base. La morte si legge **solo** come `Err(IpcError::Disconnected)` attraverso la porta, mai chiedendo alla finta: `grep -c has_died` su questo file dà **0**. Linea di base **non zero** e di **due specie**: la quota di presentazione del core, che nessun registro tiene, e un **secondo client** registrato che non muore. Due oracoli di non-vacuità, **visti rossi** entrambi |
| `crates/simulator/tests/worker_kill_campaign.rs` (**un** test — contato **sul binario** il 2026-09-02) | ⛔ **la proprietà 2 di §5.7** — nessun processo gira senza concessione valida, asserita **sui libri** dopo **ogni** kill e non alla fine. Quattro worker avviati con concessioni vere, uccisi in istanti estratti da una mescolanza **derivata**. ⚠️ La metà **temporale** di *«concessione valida»* si **conta e si dichiara** e non si asserisce — `E30`/`E39`, voce del proprietario, gotcha **#73** |
...
`crates/simulator/tests/worker_kill_campaign.rs` (proprietà **2**). **Due commit**, ciascuno
`GATE GREEN` da solo.
...
| Artefatto | Che cosa lo esercita |
|---|---|
| `ClientGrants::on_disconnect` | `crates/kernel/tests/client_grants.rs`, **quattro** sonde: la concessione torna, torna **solo quella di quel client**, un client che non tiene niente **non è un errore**, e — sonda che il compito non aveva scritto, `E154` — dopo la finestra la risposta è `AlreadyCollected` |
...
**chiamate**, non le occorrenze. ⛔ **Il comando ovvio era quello sbagliato, ed è stato colto
prima di scriverlo:** `grep -c has_died` sullo stesso file dà **2**, e i due sono il **doc di
modulo che dichiara di non chiamarla**. ⚠️ **E la seconda direzione, senza la quale uno zero
non misura niente:** lo stesso comando dà **6** su `crates/simulator/tests/dying_gui.rs`, dove
chiedere alla finta è la cosa giusta da fare.
...
| `M5` | `drain(..)` senza ripartizione — la forma che il compito suggeriva | `a_disconnect_gives_back_only_that_client_s_grants` di nuovo, **ma su un'altra asserzione** (riga 161 contro 147): rilascia le concessioni giuste e **butta la coppia del sopravvissuto**. ⛔ Gotcha **#55** risolto nella direzione che l'esclude: due mutazioni sulla stessa sonda, **assi diversi** |
...
⛔ **I DUE ORACOLI DI NON-VACUITÀ SONO STATI VISTI ROSSI, ed è un criterio di chiusura e non un
consiglio:** l'oracolo 1 con `MB3`, l'oracolo 2 con `MB4`. ⚠️ **Una sola sonda non è uccisa da
nessuna mutazione — `the_same_seed_chooses_the_same_operation` — ed è INFALSIFICABILE PER
COSTRUZIONE**, `from_seed` essendo una funzione pura dei suoi argomenti: è dichiarata tale nel
proprio corpo (gotcha **#44**), com'è la gemella in `crashing_journal.rs`, e **non conta come
copertura**.
...
📌 **Baseline a compito chiuso: `GATE GREEN`, 48 bersagli, 351 passate, 0 fallite, 2 ignorate** —
era **44 / 334 / 0 / 2**. Lo scarto è **+4 bersagli** (`client_grants`, `dying_gui`,
`gui_death_campaign`, `worker_kill_campaign`) e **+17 passate**, tutte dei banchi nuovi.
`cargo fmt --all --check` exit 0, zero avvisi su `cargo build --locked --workspace`.
...
$ grep -cE '^\| \*\*`?E[0-9]+' <piano>
    ...
    ⛔ RICHIAMO DEL 2026-09-02 — E152 NON È PIÙ APERTA: la chiude il compito 9 del
      Traguardo 6, riga 27 della tabella qui sotto. ⚠️ IL VERBALE SOPRA RESTA COM'È
      PERCHÉ NON PARLA DI CIÒ CHE È APERTO OGGI: parla del FILTRO e del suo punto cieco,
      misurato contro `git show ac65504:<piano>`, e quel punto cieco non si è mosso —
      il filtro rende zero su E152 tanto ieri quanto oggi. Ciò che invecchia è il
      PRESENTE «sono aperte», e quattro lo restano: E53, E83, E100 ed E140.
