...
⛔ **`V3` SI COPRE A METÀ, E IL CASO NEGATIVO NON È QUELLO CHE IL PIANO DETTAVA.** ⚠️ **Questa
sezione diceva *«si chiude»* e contava la riga fra le coperte:** corretto il 2026-08-20 — la
contro-sonda di catalogo è doppia e la sua seconda metà è del **Task 9**, quindi la riga è
**parziale** e il numeratore resta a **quattordici su diciannove** (`E103`). Il piano scriveva
...
| `crates/kernel/tests/arbiter_policy.rs` | **blocco C** · `V3` — la direzione *«con UNA policy compila»*, e con **entrambe** le policy: il banco costruisce un arbitro per ciascuna, da fuori la crate. ⚠️ **File nuovo del Traguardo 5 Task 8.** ⛔ **La seconda metà della contro-sonda di catalogo — *«e la transizione resta un passo giornalato (§5.4)»* — NON è qui: è del Task 9, e finché non lo è la riga `V3` è contata PARZIALE (`E103`)** | ⚠️ **sette** test — erano cinque alla nascita del file; le due dell'ondata di correzioni del 2026-08-20 sono `a_partly_full_machine_asks_back_the_need_and_not_the_whole_request` e `the_admission_asks_back_below_its_own_lane_and_spares_a_peer` (`E97`). Ricontati **sul binario** |
