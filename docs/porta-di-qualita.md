...
| `crates/kernel/tests/gateway_decisor.rs` | **blocco B** · `Q13` — la direzione *«filtrato → compila»*, che è `a_conforming_candidate_is_chosen_and_nothing_is_degraded`. ⚠️ **File nuovo del Traguardo 6 Compito 6**, e ottiene il gettone **solo** da `resolve`, mai da un costruttore di banco — che non esiste, ed è il caso `conforming_has_no_constructor.rs` a dirlo. ⛔ **Le altre sei sonde non difendono nessuna riga di catalogo:** tengono le tre uscite del filtro (ADR-0012), le due direzioni di `NoRetention` e il record risolto (ADR-0011), e a dire quali mutazioni le rendono portanti è la sezione «Il gettone di conformità del gateway» | ⚠️ **sette** test — contati **sul binario** con `cargo test --locked -p kernel --test gateway_decisor`, non per aritmetica |
