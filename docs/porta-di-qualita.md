...
| `crates/kernel/tests/gateway_decisor.rs` | **blocco B** · `Q13` — la direzione *«filtrato → compila»*, che è `a_conforming_candidate_is_chosen_and_nothing_is_degraded`. ⚠️ **File nuovo del Traguardo 6 Compito 6**, e ottiene il gettone **solo** da `resolve`, mai da un costruttore di banco — che non esiste, ed è il caso `conforming_has_no_constructor.rs` a dirlo. ⛔ **Le altre sonde non difendono nessuna riga di catalogo, e quante siano lo dice il comando in fondo a questa cella:** tengono le tre uscite del filtro (ADR-0012), le due direzioni di `NoRetention` e il record risolto (ADR-0011), e a dire quali mutazioni le rendono portanti è la sezione «Il gettone di conformità del gateway» | ⚠️ **Nessun numerale qui, e il richiamo del 2026-09-01 dice perché:** questa cella diceva *«sette test»* e il settimo giro di revisione ne ha aggiunte **due**, falsificandola nello stesso giorno in cui era stata scritta. Si contano **sul binario** — `cargo test --locked -p kernel --test gateway_decisor` — mai per aritmetica. Voce `E97` |
