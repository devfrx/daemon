...
## Errata — 2026-08-09, dopo l'esecuzione dei Task 8, 9 e 10

> ⛔ **Diciassette voci, e la maggioranza non corregge una sonda: la aggiunge.** In tre compiti
> su tre il piano prescriveva un artefatto e **nessun controllo che lo esercitasse** — il
> cablaggio di produzione «verificato eseguendo il binario a mano», le due porte verificate
> «se compila». Il resto sono derive e commenti che promettevano più di quanto la firma sotto
> mantenesse.

| # | Dove | Cosa non torna |
|---|---|---|
| **E19** | **Task 8 · Step 1 e Step 2**, il cablaggio dentro `fn main()`, verificato **eseguendo il binario a mano** | ⛔ **Nella forma del piano il cablaggio era l'unica parte del traguardo che nessun controllo esercitava.** La porta lancia `cargo build --workspace` e `cargo test --workspace`, e **mai `cargo run`** — si legge in `scripts/gate.sh`. Estratto in **`run_the_production_graph() -> Result<(), RunError>`**, che un test chiama; a `main` resta il mestiere di processo, cosa stampare e con cosa uscire, e nient'altro. Una verifica a mano è un momento nel tempo, non un controllo |
| **E20** | **Task 8 · Step 1**, il commento di `EXECUTOR_TURN_LIMIT` — *«lo scenario di riferimento, tre attività per quattro passi, usa **meno di quaranta** giri»* | ⛔ **Sono nove**, e la cifra del piano non era mai stata verificata: gotcha **#15**. Lo strumento è il limite stesso — `run` fallisce appena `turns > limit`, quindi il **più piccolo limite che restituisce ancora `Ok(())` è** il conteggio. È nove, e lo stesso nove su tutti i **duecento** semi di `crates/kernel/tests/executor_determinism.rs`: il seme cambia l'**ordine** dentro un giro, non il **numero** dei giri. Con **otto** la corsa fallisce, ed è questo che fa di nove un **confine misurato** invece di una stima. Il margine del limite spedito è perciò di **quattro** ordini di grandezza |
| **E21** | **Task 8**, nessun test | c'è un `#[cfg(test)] mod tests` **dentro `crates/daemon/src/main.rs`**, dove questo repository mette i test in `tests/` — e la deviazione **non è una preferenza, è obbligata**: la funzione è privata in un target `bin`, e un test d'integrazione è una crate a sé che può linkare **solo una libreria**. Nessun file sotto `tests/` raggiunge `run_the_production_graph`, quindi spostare il modulo fuori non lo trasferirebbe: lo cancellerebbe |
| **E22** | **Task 8**, che cosa quel test compra | ⚠️ **Non il valore del limite, ed è dichiarato nel sorgente invece che scoperto dopo.** Misurato nelle due direzioni: con `EXECUTOR_TURN_LIMIT = 0` il test resta **verde** — `run` è `while !self.tasks.is_empty()`, e senza attività il corpo non entra mai, quindi il contatore non incontra mai il limite; con un'attività mai pronta diventa **rosso**, `Err(TurnLimitReached)`. Quel che il test tiene è il **cablaggio** — il grafo si assembla e la corsa termina — non il dimensionamento del numero, che avrà il proprio controllo quando ci sarà qualcosa da far girare |
| **E23** | **Task 8**, il ramo d'errore di `main` | **residuo dichiarato**: stderr, stdout vuoto e uscita 1 sono stati verificati **a mano** e fanno tutti e tre la cosa giusta, ma nessun controllo li osserva. ⚠️ **E il prezzo si dichiara invece di sottintenderlo:** coprirlo vuol dire lanciare il binario come **processo figlio** e rileggerne i due flussi e lo stato d'uscita, per **tre righe che non decidono nulla**. È lo stesso schema con cui il `wait_until` di `platform` dichiara il proprio residuo — un controllo assente e **dichiarato** batte uno contorto — e lo scambio smette di essere equo il giorno che quel ramo si prende una decisione propria |
| **E24** | **Task 8 · Step 1**, l'ordine delle dichiarazioni dentro il cablaggio | il piano metteva le righe nell'ordine giusto e **non diceva perché**, che è come si perde un vincolo riordinando. Nel repository l'ordine è scritto come **portante** dove sta: l'`Executor` prende in prestito la cella `Sleep` per `'a` e i locali cadono in **ordine inverso** di dichiarazione, quindi invertire le due righe **non compila**. Non è stile |
| **E25** | **Task 9 · Step 6**, *«I **due** casi negativi»* | ⛔ **Sono tre.** Si aggiunge `crates/kernel/tests/compile_fail/no_conversion_from_untrusted_to_instruction.rs`, il guardiano **diretto** della regola B — *non esiste una via `From`/`Into` da `Untrusted` a `Instruction`* — che con quell'`impl` presente **compila**, e `trybuild` lo riporta come `error` senza passare da nessun oracolo. **Misurato portante:** con `impl From<Untrusted> for Instruction` scritto **e quel caso rimosso**, la porta esce **verde su sei controlli su sei** mentre I6 è già caduta |
| **E26** | **Task 9 · Step 7**, la ragione con cui il caso della regola A doveva reagire a quella conversione | ⚠️ **Era sbagliata, e la divergenza si registra invece di allinearla all'attesa.** Il gotcha **#42** prevedeva un **`mismatch`** — rustc che appende righe di `help: call Into::into` che l'oracolo non porta. **Misurato su questa coppia: resta `ok`**, l'uscita `E0308` combacia ancora esattamente con l'oracolo, e nulla da nessuna parte diventa rosso. Lo scarto qui è fra **riferimenti**, e `From<Untrusted> for Instruction` non produce nessun `&Untrusted: Into<&Instruction>`, quindi rustc non ha suggerimenti da appendere; sui due tempi della §2.1 lo scarto era fra valori **posseduti**, e lì il suggerimento compariva. 📌 La conclusione punta nella stessa direzione, più forte: quella guardia non è *disarmabile da una rigenerazione dell'oracolo*, è **cieca dalla nascita** |
| **E27** | **Task 9 · Step 1**, `StepId` con `PartialOrd, Ord, Hash` e un metodo `get()` | derive ridotti a `Debug, Clone, Copy, PartialEq, Eq` e **`get()` cancellato**: niente ordina o indicizza questi valori e nessuno legge il numero, che è la regola con cui erano già spariti `Millis::ZERO` e `Monotonic::as_millis` (E6). ⚠️ `Hash` in particolare sarebbe un derive **il cui consumatore abituale non può esistere qui**: `HashMap` non è nominabile in questa crate — `tests/compile_fail/hashmap_in_kernel.rs`, gotcha #12. Tornano il giorno che servono, col chiamante che li chiede |
| **E28** | **Task 9 · Step 4**, `#[derive(Debug)]` su `Untrusted` | `Debug` è **scritto a mano** e **non stampa il contenuto** — solo `Untrusted(<N bytes>)`. Non è cosmesi: col derive, `Instruction::new(format!("{:?}", untrusted))` portava il testo esterno oltre il confine intatto e **nulla diventava rosso**. Ed è la stessa classe di problema del canale delle istruzioni — contenuto che nessuno ha scelto, che arriva dove viene letto come se qualcuno l'avesse scelto. ⚠️ La lunghezza resta apposta, e la via chiusa è **pinnata** da `the_debug_of_untrusted_does_not_print_the_content`: una via chiusa che nessun test tiene si riapre il giorno che qualcuno rimette `Debug` nella lista dei derive, con la porta verde |
| **E29** | **Task 9**, il titolo del compito e la frase *«la conversione … che è **giornalata**»* | ⛔ **Sostituita da un residuo dichiarato, perché letta come garanzia è falsa.** Il meccanismo compra **una cosa sola**: che la conversione non si scriva senza **nominare** la porta. **Non** compra che qualcosa sia stato registrato — `promote` è generico su qualunque `Journal`, e un'implementazione che risponde `Ok(())` senza scrivere un byte soddisfa il vincolo. L'onestà di un'implementazione non è una proprietà di livello 1 e non lo diventa mai: la tiene una **suite di conformità**, come `tests/reactor_contract.rs` fa col `Reactor` e i suoi due bugiardi, e quella di `journal` è del Traguardo 3, con il record durevole su cui confrontarla. ⛔ Le vie che aggirano il confine **e compilano oggi** sono **sette**, elencate una per una su `Untrusted::promote` col prezzo di chiudere ciascuna; **una sola è chiusa**, la A3 di E28. Il guardiano copre le vie che esistono: non è totale, e ora lo dice |
| **E30** | **Task 9 · Step 5**, *«4 passed»* | i test di `crates/kernel/tests/boundary_promotion.rs` sono **otto**. I quattro nuovi non sono ornamento: il passo e la ragione **riletti** dal giornale, perché contare i record lascia verde una `promote` che registra il passo sbagliato (gotcha #30); la troncatura di `summarize` nelle **due** direzioni; il `Debug` di E28. ⚠️ E il test che il piano chiamava `the_label_is_hereditary` è stato **spezzato in due**: asseriva sul testo troncato e sul nome prometteva l'ereditarietà dell'etichetta, che invece la tiene **l'annotazione di tipo** — cioè il compilatore — e non un'asserzione |
| **E31** | **Task 10**, *«Nessuna implementazione»*, e uno Step 3 che verifica **solo che compili** | ⛔ Esiste **`crates/kernel/tests/ports_are_implementable.rs`**: quattro test, una finta per ciascuna delle due porte, con le chiamate che le esercitano nelle **due direzioni** — riuscita e rifiuto. È il rimedio al gotcha **#46**: un tratto che **nessuno implementa non è provato implementabile**, perché una firma può essere inusabile per i prestiti, per il possesso o per la sicurezza d'oggetto, e lo si scopre quando il codice che deve usarla esiste già. ⚠️ In **questo stesso traguardo** era già successo: la variante `Wakeup` dichiarata in anticipo si è rivelata inusabile ed è stata tolta (E8, E9) |
| **E32** | **Task 10 · Step 1 e Step 2**, `Path` e `CheckpointId` con `PartialOrd, Ord, Hash`, e `CheckpointId::get()` | derive ridotti e `get()` **cancellato**, per la regola di E27 — e i derive rimasti portano scritto accanto **chi li obbliga**: `Clone` su `Path` per `declare_scope`, che consegna una fetta in prestito che l'implementazione deve trattenere, `Copy` su `CheckpointId` perché la maniglia entra nel deposito **ed** esce dalla chiamata. ⛔ **Ma `Path::as_bytes()` ed `Endpoint::as_bytes()` sono stati TENUTI, e senza il test di E31 la stessa regola li avrebbe cancellati**: la privacy del campo di una tuple-struct è **di modulo**, quindi senza accessore un'implementazione fuori da `kernel` non potrebbe né consegnare il percorso all'OS né comporre l'indirizzo da chiamare — le due porte sarebbero **non implementabili fuori dalla crate che le dichiara**, e nessuno se ne accorgerebbe finché non tocca scriverle. È il gotcha **#46** nella sua forma esatta |
| **E33** | **Task 10 · Step 2**, il commento di `request` — *«nulla attende dentro `network`»* | la frase resta, e **accanto** c'è ciò che il piano non diceva: la firma di `request` è **sincrona e bloccante** — restituisce l'intera risposta, quindi qualcuno ha atteso — e l'esecutore di questo kernel è **cooperativo**, dove una chiamata bloccante nel percorso decisionale ferma ogni altra attività. Le due cose non stanno insieme, e la tensione è scritta come **domanda aperta dichiarata** invece che sciolta: la firma viene dalla spec e questo file **non ha l'autorità per rifarla**, e inventarne una adesso ne congelerebbe una che nessuno ha misurato — la regola di ADR-0009, la stessa che ha tolto `Wakeup`. ⚠️ Oggi non è un difetto perché **non c'è nessun chiamante**; va riconciliata il giorno che la porta riceve un'implementazione |
| **E34** | **Task 10 · Step 1**, `restore` documentato come *«rilegge una versione conservata»* | ⚠️ La firma **restituiva già `()`**, quindi il commento descriveva una cosa che la firma non fa — ed è il modo in cui un lettore si costruisce una garanzia che non c'è. Riscritto: un ripristino è un **effetto sul file**, e chi volesse i byte in mano starebbe **leggendo**, e leggere è `read` |
| **E35** | **Task 10**, le varianti d'errore senza produttore | **non** sono state potate, e la ragione è scritta nel sorgente proprio perché il caso **assomiglia** alla regola che ha tolto `Wakeup::EventReady`. Là quella variante non poteva avere **nessun produttore possibile** — niente in questo traguardo genera eventi esterni. Qui **nessuna** variante ha un produttore, per il motivo piano che la porta **non ha implementazione affatto**: applicare la regola su quella base **svuoterebbe** l'enum invece di potarlo. Ciascuna delle tre è un guasto che un deposito vero ha davvero |

📌 **E ciò che distingue questo giro dai due precedenti si dice in una riga.** Nei Task 1–2 e nel
Task 7 il difetto stava quasi sempre **nella sonda** — sbagliata (E1, E2, E7), collocata nella
sede sbagliata (E14), o nata non provata mentre rimediava a un'altra (E15). Qui, **tre volte su
tre**, il difetto è stato che il piano **non prevedeva alcuna sonda**: nessun test per il
cablaggio di produzione (E19, E21), nessuno per le due porte dichiarate (E31). ⛔ **E una sonda
che manca non si vede leggendo**, perché non c'è niente da leggere: una sonda sbagliata la si
coglie confrontandola con ciò che dovrebbe far scattare, un'assenza la si coglie **solo
chiedendosi, per ogni artefatto che il compito produce, quale controllo lo eserciti**. È la
domanda che ha prodotto `run_the_production_graph` e `ports_are_implementable.rs`, e nessuna
rilettura del piano l'avrebbe posta al posto nostro.
