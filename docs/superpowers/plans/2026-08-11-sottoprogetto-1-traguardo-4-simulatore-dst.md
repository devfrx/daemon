...
| **E67** | ⛔ **La mutazione dello Step 3 nomina una sonda che NON ESISTE PIÙ.** Detta di forzare un rosso dentro `the_short_campaign_runs_on_every_commit`: quel nome è stato **assorbito dal Task 4**, dove `c7b_a_crash_leaves_exactly_the_steps_the_scenario_left_open` **è** la campagna breve. È il **contratto che cresce sotto il piano**, la quinta domanda del pre-controllo, e stavolta a farlo crescere è stato un compito dello stesso piano |
| **E68** | ⛔ **Lo Step 4 era in gran parte GIÀ ESEGUITO — gotcha #49 per la terza volta in questo progetto, e previsto dal compito stesso.** Le righe di registro dei quattro artefatti erano già state scritte dai compiti che li avevano prodotti; a mancare non era l'aggiunta ma **il riconteggio**, e uno c'era davvero: la riga di `dst_campaign.rs` diceva **quattro** test mentre sono **cinque** — la cifra è rimasta ferma quando il **Task 4** ha aggiunto la campagna profonda. Più il nome della sonda del #51, che il registro descriveva senza nominare |
| **E69** | ⚠️ **Il passo del cancello aveva bisogno di una sede propria, e la tabella dei controlli non lo era.** `gate.sh` conta i **fallimenti**, non i passi, quindi aggiungerne uno non fa dire «sette» a nessuna cifra — ma **può** diventare rosso, e allora *«non è un settimo controllo»* va argomentato invece che dichiarato: non può fallire per una ragione che il secondo controllo non abbia già colto, e la sua **doppia rossa** è l'unica prova che esegua ciò che dice. Registrato in una sottosezione propria, con le due direzioni misurate |
| **E70** | ⚠️ **I due prefissi delle righe di tempo di parete non concordavano** — `DST L1 campaign:` contro `L2 short:` — e quelle righe esistono **per essere lette come coppia** dentro l'uscita del cancello. Allineati nello stesso passaggio: due grafie per una cosa sola costringono chi scorre l'uscita a conoscerle entrambe |
