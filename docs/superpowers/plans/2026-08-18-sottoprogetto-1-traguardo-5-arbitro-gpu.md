...
| **E79** | Task 7, in seconda revisione | ⚠️ **UN'ASSERZIONE CHE NON PUÒ FALLIRE — TENUTA, non cancellata, ed è la specie di `E37`.** In `asking_back_marks_nothing_when_the_reclaimable_does_not_cover_the_need`, l'`assert_eq!(arbiter.allocated(), Mib::new(8_192))` è vera per **qualunque** implementazione di `ask_back`: quella funzione non toglie **mai** dai libri — solo `collect_expired` lo fa, e allo `ORIGIN` con finestre `LONG` non c'è niente da riscuotere. ✅ **Riprodotto misurando e non dedotto**, isolandola (le due asserzioni sopra tolte) e rieseguendo tre mutazioni: resta **verde** sotto la **12**, che è il suo **uccisore solo** — 13 passate 0 fallite, dove a piena forza è 12/1 — sotto la **8** (12/1, e a morire è un'altra) e sotto la **1** (9/4, e questa sonda non è fra le morte). ⛔ **Il rimedio di questo repository NON è cancellarla:** dichiara l'intento che il **nome** porta — che i libri non si sono mossi — e torna a mordere il giorno in cui `ask_back` avesse una via che li tocca. Cancellare un'asserzione perché oggi è verde è il modo in cui quel giorno arriva senza che nulla lo dica. Le si è scritto **accanto** che cosa tiene davvero: le due asserzioni sopra, la **risposta** e lo **stato** |
| **E80** | Task 7, in seconda revisione | ⚠️ **LA CAMPAGNA È STATA RIMISURATA DA CAPO PER LA SECONDA VOLTA NELLO STESSO GIORNO, e la rimisura ha corretto una MUTAZIONE oltre ai numeri.** ① La suite passa da **dodici** a **tredici** sonde nel bersaglio `lib`, quindi tutte le venti righe portavano di nuovo i conteggi di una suite che non esiste più (gotcha **#31**); rimisurate **tutte**, mai dedotte. Le tre righe **`D`** restano l'unica eccezione e sono **datate**: ✅ verificato che la data ci sia, misurano la forma **dettata dal Passo 3** che con le due passate non esiste più in versione confrontabile (`E35`). ② ⛔ **La riga 1 era una mutazione più DEBOLE di quella che la sua cella descrive:** il `retain` era stato messo solo **in coda**, dove l'uscita anticipata `return covered` lo saltava — misurata così dà **9/4** invece di **6/7**. Rifatta con `break 'lanes`, perché la riscossione avvenga **sempre**. Una mutazione che non fa ciò che la sua riga dice è un mutante vivo travestito da misura. ③ ✅ **La riga 13 (`values_mut().rev()`) è stata RIMISURATA contro il codice di oggi e non citata** — era stata scritta **prima** della riscrittura in due passate — ed è **ancora un mutante vivo**: `lib` **13 passate, 0 fallite**, `arbiter_admission.rs` **20, 0**. La politica resta aperta (`E70`). ④ ⛔ **UNA PERDITA DI ESCLUSIVITÀ, RIMEDIATA NELLO STESSO GIRO:** la sonda nuova di `E75` muore anche sotto la **10**, quindi `asking_back_takes_the_worst_lane_first` ha perso l'uccisore solo — e **non** era per questo dominata, il suo insieme non è contenuto in nessun altro. ✅ **Candidata isolante cercata e TROVATA — riga 10b**, *«una vittima più grande del bisogno viene saltata»*: **rossa e SOLA, 12 passate 1 fallita**. Le due sonde condividono lo scenario e differiscono per la **taglia del bisogno**, non per l'ordine — quindi nessuna mutazione dell'**ordine** poteva separarle, e una della **scelta della vittima** sì; in più è l'unica riga che eserciti *«marcare può eccedere»*. Bilancio: **nove** sonde su **quattordici** con uccisore solo (erano otto su tredici), campagna a **venticinque** righe |
