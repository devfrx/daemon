...
| **D** — i meccanismi | 5, 6, 7, 8 | ▶️ **IN CORSO** — il **compito 5 è ESEGUITO** il 2026-09-01, `GATE GREEN` a ciascuno dei tre commit. Il pre-controllo aveva reso **cinque** voci, `E44`–`E48`, **due bloccanti**; eseguendo ne sono uscite altre **tre**, `E49`–`E51`. ⚠️ **E il compito 5 porta un RESIDUO DI METODO, `E53`: è stato eseguito in sessione e NON ha avuto il ciclo di revisione indipendente** che questo piano prescrive in testa. Chi riprende lo eredita, e non si chiude eseguendo il compito 6. Scritta il 2026-08-30 — pre-controllo P-12…P-15, voce chiusa dalla **D22**, e i quattro compiti |
...
| **E53** | ⛔ **IL COMPITO 5 È STATO ESEGUITO IN SESSIONE E NON SUBAGENT-DRIVEN, E IL CICLO DI REVISIONE INDIPENDENTE NON C'È STATO. È IL RESIDUO DICHIARATO DI QUESTO COMPITO, e chi riprende lo eredita insieme al prodotto.** L'intestazione di questo piano dice *«SOTTO-SKILL OBBLIGATORIA — `superpowers:subagent-driven-development`, un subagente fresco per compito con revisione fra uno e l'altro»*; il 2026-09-01 la sessione aveva l'uso dei sotto-agenti **disabilitato** salvo richiesta esplicita del proprietario, e la richiesta non è arrivata — a una domanda diretta su quel permesso è stato risposto *«decidi secondo i criteri»*, che **non è** il permesso. ⚖️ **Scelto di eseguire in sessione dichiarando il costo, invece di fermarsi:** il pre-controllo aveva già trovato **due voci bloccanti**, quindi fermarsi avrebbe lasciato il compito con i difetti noti e nessun rimedio. ⛔ **CHE COSA SI PERDE, misurato sui precedenti e non temuto in astratto:** in questo repository è **la revisione** ad aver trovato i mutanti vivi che l'esecuzione non vedeva — il `promote` che non serviva la corsia `Realtime` con **34 target su 34 verdi** (gotcha **#74**), i due argomenti di `ask_back` tenuti da nessuna sonda su uno scenario degenere, la via di `FromWorker::decode` che nessuna sonda raggiungeva. Un revisore fresco legge il prodotto **senza sapere che cosa l'autore intendeva**, ed è precisamente quel non-sapere a comprare i rilievi. ✅ **CHE COSA È STATO FATTO AL SUO POSTO, col limite dichiarato:** le **cinque** mutazioni prescritte sono state eseguite una per una e revocate da copia — e una, `M3`, era **viva** ed è stata chiusa (`E51`); il criterio di chiusura è stato riletto punto per punto contro il consegnato, ed è da lì che è uscita `E52`. ⛔ **Ma è una rilettura fatta da chi ha scritto il codice, e non vale una revisione:** i difetti che restano sono per costruzione quelli che l'autore non vede. 📌 **IL PROSSIMO PASSO, eseguibile:** se il proprietario autorizza i sotto-agenti, si apre **il ciclo di revisione sul compito 5 PRIMA di dispacciare il compito 6** — il perimetro è `f3ea37f..8aaa78c`, cioè i tre commit di prodotto più le due passate di documentazione. Se non li autorizza, la voce resta **aperta e dichiarata**, e chi riprende sa che quel prodotto ha meno revisioni di ogni altro compito di questo traguardo. ⚠️ **Non si chiude eseguendo il compito 6:** un compito nuovo non rivede quello prima. |
