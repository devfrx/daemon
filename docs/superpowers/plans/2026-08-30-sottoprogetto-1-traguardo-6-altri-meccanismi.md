...
| **E46** | ⚠️ **LA CIFRA DI `P-13` NELLA PROSA DELLA PARTE D È STANTIA, E IL PIANO SE NE DIFENDE GIÀ — ma la cifra resta scritta.** Il capoverso d'apertura della Parte D prezza *«ventisei siti di costruzione»*; ✅ **rimisurato il 2026-09-01** col comando che il Passo 5 del commit 5b ordina di rilanciare, `grep -rn 'RecordV1 {' crates/ --include=*.rs`: rende **39** righe, di cui **31** fuori dai commenti e **8** dentro. ⛔ **Il metodo regge e il difetto è solo nella prosa:** il passo dice già *«Non fidarti del numero di P-13: rilancia il comando»*, quindi chi esegue non può sbagliare — ma un lettore che si ferma alla Parte D prezza il compito su un numero che non regge, ed è la radice **R1** dell'audit del 2026-08-27. 📌 **Rimedio: la cifra si TOGLIE e resta il comando**, come `CLAUDE.md` prescrive e come questo stesso piano ha già fatto con la lista dei commit e col totale dell'errata. ✅ **E il numerale «il 34° caso `compile_fail`» REGGE**, ricontato con lo stesso metodo: `ls crates/kernel/tests/compile_fail/*.rs` ne dà **33**. |
