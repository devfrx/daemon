...
| **4** | il passaggio unico sui documenti di stato, **l'unico che dice «chiuso»** | uno | ✅ 2026-09-03 |
...
| **E18** | ⛔ **COMPITO 4, Passo 2 — `roadmap.md` DICE CHE IL PIANO DEL TRAGUARDO 6 NON È SCRITTO, E IL PASSO 2 GLI AGGIUNGE LA RIGA DUE CAPOVERSI SOPRA.** Misurato il 2026-09-03: il capoverso sotto la tabella dei piani diceva *«quelli dei Traguardi 2, 3, 4 e 5 sono scritti … quello del sesto ancora no»*, mentre `docs/superpowers/plans/2026-08-30-sottoprogetto-1-traguardo-6-altri-meccanismi.md` esiste ed è **eseguito** — e il Passo 2 mette proprio quel piano nella tabella sopra. Senza rimedio il commit del compito 4 avrebbe consegnato un file che si contraddice a tre righe di distanza. ⚠️ **E il difetto è alla SECONDA occorrenza nella stessa riga**: sotto di essa vive già un richiamo dell'audit del 2026-08-11 — *«diceva «quelli dal quarto in poi ancora no» con il piano del quarto scritto ED ESEGUITO»* — cioè la stessa specie, corretta una volta riallineando l'elenco invece di toglierlo. 📌 **Rimedio, applicato e dentro il perimetro** (`roadmap.md` è del compito 4): l'enumerazione è **TOLTA** e non riallineata — *«quali siano scritti lo dice la tabella qui sopra»* — sui precedenti **AUD-007** e **AUD-046**, e il richiamo datato di una riga la registra (vincolo globale 9). Trovata dal pre-controllo del compito 4, domanda **5** — *il contratto cresce sotto il piano* |
| **E19** | ⚠️ **COMPITO 4, Passo 8 — LE RIGHE 6 E 7 RENDONO FALSO IL TITOLO DELLA §7.2, CHE DICE «LE CINQUE CONDIZIONI».** Il Passo 8 detta di aggiungere le righe **6** e **7** alla §7.2 del disegno, e la §0.7 della spec porta **sette** condizioni — `sed -n` sulla sua tabella le conta tutte e sette. Il titolo `### 7.2 Le cinque condizioni della §0.7` diventa falso **nello stesso commit** che lo rende tale, e con esso il capoverso *«Le righe 6 e 7 le scrive il compito 4»*. 📌 **Rimedio, applicato:** il numerale è **tolto** dal titolo — `### 7.2 Le condizioni della §0.7, rilette contro il codice` — e il capoverso diventa un **richiamo datato**; il numero di sezione `7.2` **non** cambia, quindi il controllo dei duplicati di `check-docs.sh`, che legge `^#{2,3} <numero>`, vede la stessa chiave di prima. ⚠️ **Il numerale nel titolo era corretto quando il compito 1 lo scrisse**, e questa non è una critica a quel compito: è la specie del gotcha **#49** — un metro scritto per uno stato che il lavoro poi muove. ⛔ **RICHIAMO DEL 2026-09-03:** il pattern citato qui sopra, `^#{2,3} <numero>`, è **superato** — `scripts/check-docs.sh` legge `^#{2,6} [0-9]+(\.[0-9]+)*` (riga 101; rilanciato il 2026-09-03 con `grep -n "grep -ohE" scripts/check-docs.sh`), e le occorrenze di `^#{2,3}` nello script sono **commenti** che spiegano perché il vecchio era cieco a `####` e troncante al secondo gruppo, corretto il 2026-08-27 col finding **AUD-025**. **La conclusione regge sotto entrambi** — la chiave resta `### 7.2` in tutti e due — e **non si riscrive**. Trovata dal pre-controllo del compito 4, domanda **5** |
