...
**Forma.** Nessuna riga di prodotto: la chiusura è un **audit** più tre passate documentali.
Quattro compiti in sequenza, un commit che dice «chiuso» soltanto alla fine. Le decisioni sono
...
| # | Vincolo | Da |
|---|---|---|
| 1 | **la spec non si tocca**: `git diff --name-only <base>..HEAD -- docs/superpowers/specs/2026-08-06-sottoprogetto-1-kernel.md` deve restare vuoto | disegno §6.3 condizione 7 |
...
⛔ **Nessun file di `crates/` cambia in nessun commit**, e il compito 4 lo prova col comando del
vincolo globale 1 esteso a `crates/ scripts/ Cargo.lock`: deve nominare **solo**
