...
| **E11** | **Compito 7, Passo 2 — il criterio di S2 nomina P2 e lo prova su una statistica più DEBOLE, e il massimo non lo riporta nessuno.** Trovata dalla **revisione** del compito 7, il 2026-09-04, e rimisurata dal coordinatore prima di portarla: `spikes/GUI-REQUISITI.md` riga 77 enuncia P2 come *«ritardo **massimo** fra emissione e rendering — < 100 ms»*, e la riga 103 lo misurò come **picco** — mentre `spikes/gesti/PROTOCOLLO.md` lo prova su un **p95** e la sua riga *«Che cosa si riporta»* chiede **solo mediana e p95**; `page.html` non calcola nessun massimo. ⛔ **E l'asimmetria è dentro lo stesso documento:** la riga di S1 riporta già *«mediana, p95, massimo»*. Conseguenza: su 600 campioni un p95 lascia passare fino a trenta campioni sopra i 100 ms senza che il criterio cada, e l'esito di SP-7 finirebbe accanto alla riga P2 di SP-5/SP-6 **senza essere confrontabile con essa**. ✅ **DECISO DAL PROPRIETARIO il 2026-09-04, non dal coordinatore:** il **criterio non cambia** — resta `p95 < 100 ms`, che è ciò che la §4.2 del disegno approva, *«nessuna soglia inventata»* — e si aggiunge il **massimo** a *«Che cosa si riporta»* di S2 e al `dump` di `page.html`, con un **richiamo datato** nel protocollo che dichiara la modifica al metro, come la sua clausola *Congelamento* pretende. ⚠️ **Portata al proprietario invece che decisa eseguendo** per tre ragioni scritte: il protocollo è **congelato** dal commit del codice; il metro è un criterio che lui ha approvato nel disegno; e il compito 8 lo vuole comunque, quindi la domanda non è costata una sessione ferma. ⛔ **E va decisa PRIMA della misura:** scegliere il metro conoscendo il risultato è ciò contro cui *«criteri scritti prima della misura»* esiste. Domanda **1** — la sonda attaccava il **numero** di P2 e non la sua **statistica** |
| **E12** | **Compito 7, Passo 5 — le cinque righe di `.gitignore` non coprono il bytecode Python, e il compito 8 lo ricrea.** Trovata dal **coordinatore** il 2026-09-04 prezzando il `DONE_WITH_CONCERNS` del compito 7, e non dalla revisione: `grep -n 'pycache' .gitignore` non rende **nulla**, e le cinque righe dettate — `.venv/`, `relay/target/`, `relay/Cargo.lock`, `*.task`, `*.csv` — non lo prendono. L'esecutore del compito 7 ci è già caduto una volta: una verifica con `py_compile` gli ha lasciato `spikes/gesti/__pycache__/` nell'albero, tolto a mano prima del `git add`. Il **compito 8 lancia** `s1_bench.py` e `s2_worker.py`, che lo ricreano. ✅ **DECISO DAL PROPRIETARIO il 2026-09-04:** si aggiungono `__pycache__/` e `*.pyc` in una **sezione propria**, non dentro il blocco degli spike, perché non sono artefatti di uno spike — il codice dei worker sarà Python (ADR-0028). Domanda **2** — per l'artefatto *«l'albero dopo una corsa dello spike»* nessun controllo lo teneva |
| **E13** | **Compito 8, Passo 7 — l'oracolo `grep -c '<' spikes/RISULTATI.md` NON può rendere «quanto rendeva prima della sezione», e non è una svista di conteggio ma una sonda che attacca il bersaglio sbagliato.** Trovata **eseguendo** il compito 8, il 2026-09-04, misurando **prima** di scrivere la sezione. `grep -c` conta le **righe**, e la sezione che il Passo 5 detta ne introduce due che portano legittimamente un `<`: i criteri *«mediana e p95 < 33 ms»* e *«relay → pagina p95 < 100 ms»*. Misurato: **6** prima, **8** dopo, e le due in più sono esattamente quelle. ⛔ **E la forma stretta `grep -c '<[^ ]'` non serve come oracolo globale:** rende **2** già prima della sezione, su due righe di SP-6 che citano la sintassi TypeScript di un aggiramento — contenuto vero, non segnaposto. ✅ **Si detta corretto, e l'oracolo si delimita alla SEZIONE invece che al file**, dove il valore atteso è **0** e non dipende da ciò che il resto del documento contiene: `awk '/^## SP-7 /{s=1;next} s&&/^## /{s=0} s' spikes/RISULTATI.md \| grep -c '<[^ ]'`. Il resto del Passo 7 non cambia. Domanda **1** — la sonda attaccava il **file** invece della **sezione**, ed è la stessa specie di **E7** |
| **E14** | **La sezione SP-7 di `spikes/RISULTATI.md` è stata RIVISTA il 2026-09-04, dopo il compito 8, per decisione del proprietario — ed era l'unico artefatto del piano che nessuno aveva riletto.** Il compito 8 non fu dispacciato a un subagente: l'eseguì il coordinatore col proprietario alla telecamera, e il compito 9 ne **copia l'esito** nei documenti di stato. Un revisore in sola lettura, con l'ordine di rilanciare ogni comando accanto a un'affermazione misurabile: **2 Important, 6 Minor**. ⛔ **I due Important, riverificati dal coordinatore prima di correggerli.** ① `docs/roadmap.md` riga 6 diceva *«Ultimo aggiornamento: 2026-09-03»* mentre `git log -1 --format=%ad --date=short -- docs/roadmap.md` rende **2026-09-04** — e il richiamo che quella riga già ospita **prevedeva** l'occorrenza (*«il rimedio non dice ancora CHI la riallinea … la prossima occorrenza è prevista, non esclusa»*): si è avverata al **primo** tocco successivo, e a coglierla è stata una revisione e non chi ha toccato il file. Riallineata, sul precedente di **AUD-034** e **AUD-039**; la **cura strutturale** — un controllo che confronti quella data con `git log` — è una **riga di catalogo nuova**, quindi **del proprietario, registrata e non presa**. ② L'osservazione **O4** spiegava il costo della perdita di tracciamento con *«MediaPipe rilancia il rilevatore del palmo, che è la parte pesante»*: **nessuna misura di quella sessione la produce** e `riferimenti.md` non la traccia — tre righe sotto, **O5** rifiuta esplicitamente di inventare una causa per gli ~80 ms, cioè la stessa tabella applicava **due metri opposti**. La frase è **TOLTA** e al suo posto sta il metro applicato (gotcha **#76**: si toglie, non si riscrive meglio). ⚠️ **Quattro Minor presi, due no.** Presi: **O2** contava i fotogrammi al secondo della webcam, che il banco **non misura** — `grep -n CAP_PROP_FPS spikes/gesti/s1_bench.py` non rende nulla, e `results/s` esce da `len(latencies_ms) / elapsed_s` — quindi la mezza frase è tolta; l'intestazione della sezione diceva *«congelato … prima della misura»* **tacendo** il richiamo datato che il compito 7 ha messo nel protocollo (voce **E11**), e ora lo nomina; le due righe di evidenza della copia usa-e-getta **nominano** le due righe di differenza, così che restino riderivabili benché la copia sia cancellata; e la cella di **E13** qui sopra portava un `\|` non escapato dentro un code span, che spezza la cella. ⛔ **Non presi, e la ragione è scritta:** la cifra `7819105` vive anche nella sezione *«Come si riprende»* di questo piano, che è un **verbale datato** e non si corregge (55ª misura); e la **cadenza di S2 e il numero di mani in campo** non sono registrati da nessuna parte — il criterio di S2 non ne dipende, ma il lettore non può escluderlo, e resta **dichiarato qui**. 📌 **Nessun gotcha nuovo:** i due Important sono il **#31** (una data al presente che nessuno fa rispettare) e il *«mai inventare»* di `CLAUDE.md`, entrambi già scritti |
| **E15** | **Compito 9, Passo 1, condizione 3 — il comando è la forma ROTTA che E7 ha già corretto per il compito 4.** Trovata dal pre-controllo del compito 9, il 2026-09-04: `grep -n -E '^\| \*{0,2}[0-9a-z]{1,3}\*{0,2} \|' docs/roadmap.md` pesca anche la tabella dei consumatori di ADR-0033 — le sue ultime righe sono `\| 1 \|`, `\| 2 \|`, `\| 3 \|` — quindi non mostra affatto la coda della tabella dei sotto-progetti. **Si detta corretto con la forma di E7, delimitata per sezione:** `awk '/^## Sotto-progetti/{s=1;next} s&&/^## /{s=0} s&&/^[\|] /' docs/roadmap.md \| tail -4`, che rende oggi `9`, `10`, `11`, `12` — la riga 12 in coda e 0…11 nell'ordine di prima, che è ciò che la condizione chiede. Domanda **1**: la sonda attacca il **file** invece della **tabella**, ed è la **terza** occorrenza dello stesso difetto in questo piano dopo E7 ed E13 |
| **E16** | **Compito 9, Passo 1, condizione 6 — l'attesa dice «cinque righe» e il comando ne rende QUATTRO.** Stesso pre-controllo: `grep -c '^## SP-7\|^### SP-7' spikes/RISULTATI.md` rende **4**, e sono esattamente ciò che il modello del compito 8 detta — un `## SP-7 —` più i tre `### SP-7 ·` (*Osservazioni*, *Versioni*, *Evidenze*). Il **criterio di chiusura del compito 8**, nello stesso piano, dice infatti *«le quattro sottosezioni»*: le due cifre si contraddicevano fra loro. **Il valore atteso è `4`**, e la condizione non cambia nel merito. ⚠️ **L'attesa non era mai stata contata**, che è la specie di difetto di nove voci d'errata su nove dei piani precedenti |
| **E17** | **Compito 9 — QUATTRO correzioni vere che la sua lista `Modify:` non nomina, e il proprietario ne ha deciso la forma il 2026-09-04: RICHIAMI DATATI accanto alla frase**, non voci d'errata, perché tre delle quattro vivono nel **disegno** e chi legge il disegno non legge l'errata di questo piano. Le prime tre sono le voci che le sessioni dei compiti 4–5 e 8 hanno rimandato qui, tutte e tre **riverificate dal pre-controllo**: ① il **disegno** attribuisce ad **AUD-032** la forma *«il richiamo in testa all'ADR»* in tre punti — righe 128, 269 e 547 — mentre **E4** di questa errata ha misurato che il rimando di AUD-032 sta **dentro la Decision** di ADR-0005, accanto al paragrafo che corregge; ② il **disegno non nomina mai `float16`** — `grep -n 'float16'` su di esso non rende **nulla** — mentre l'URL che la §7 traccia in **F4** e la tabella delle versioni di SP-7 lo portano entrambi: la §6.2 e la §7 non dicono **quale bundle** si usa; ③ la cella *«due (D6)»* della colonna *Commit* del **compito 7**, nella tabella della posizione di questo piano, mentre `git log --oneline 43ce0a0..311bce0` conta **tre**. ④ **E una quarta, nata da E14:** il Passo 4 del compito 9 tocca di nuovo `docs/roadmap.md`, quindi la sua **riga 6** va riallineata **nello stesso commit** — o il difetto che E14 ha appena chiuso si riapre alla terza occorrenza, che è precisamente ciò che il richiamo di quella riga prevede. ⛔ **`spikes/RISULTATI.md` NON entra nel compito 9:** le sue correzioni sono già committate dall'ondata di E14 |
| **E18** | **La REVISIONE del compito 9 — l'ultimo del piano — ha trovato 1 Important e 5 Minor, e l'Important era una cifra sparsa in più case.** ✅ **Le nove condizioni della Definizione di «fatto» passano tutte**, rilanciate dal revisore con la forma di **E15** e il valore di **E16**; i quattro richiami di **E17** sono **veri e provati coi comandi**; e il **puntatore del prossimo passo vive in una casa sola** — `grep -n '⏭️' docs/COMPENDIO.md` rende **due** righe (il puntatore vivo in §6 e la riga della tabella delle voci aperte che lo **nomina**), e fuori dal compendio **nessun documento riscrive il passo**: i puntatori di [`README.md`](../../README.md), `roadmap.md` e `HANDOFF.md` **rimandano** alla §6, e le altre righe col marcatore lo **nominano** come dato — dentro un gotcha, un comando o un'etichetta d'errata. ⛔ **L'Important:** il **disegno** diceva *«le DODICI decisioni»* in **tre** punti vivi — l'etichetta sopra la tabella, la §5.4 e la voce 3 delle voci aperte — mentre quella tabella ne porta **tredici**, e il compito 9 aveva appena scritto *«tredici»* nei **due indici** che mandano il lettore a quel disegno: la §12 del compendio e la tabella *«Specifiche»* di [`README.md`](../../README.md). La contraddizione era **dentro** il disegno dal 2026-09-03 — la decisione 13 dichiara di sé di essere nata quel giorno, dopo la consegna — e il commit del compito 9 l'ha **spostata fuori**, il che è precisamente ciò che rende un indice pericoloso. ✅ **La cifra è TOLTA dalle cinque case vive FUORI dal piano**, non riallineata a tredici: la casa unica è **la tabella**, col comando accanto. La sesta, dentro la sezione *«Come si riprende»* del disegno, **non è toccata** — è un **verbale datato**, e un verbale invecchia onestamente (55ª misura). ⛔ **E LA PRIMA STESURA DEL RICHIAMO HA COMMESSO IL DIFETTO CHE CORREGGEVA:** il comando era ancorato ai **numeri di riga**, e il richiamo **stesso** ha spostato la tabella facendogli rendere **5** invece di 13 — colto **prima del commit**, rilanciandolo invece di fidarsene. Riscritto delimitato per **testo**, e provato nelle **due direzioni**: cinquanta righe inserite sopra → ancora **13**; una riga di tabella in più → **14**. 📌 *Un numero di riga è un numero, e marcisce come tutti gli altri* — è il gotcha **#31** applicato a un comando invece che a una cifra. ⚠️ **Dei cinque Minor ne sono presi DUE, due sono dichiarati e uno era un rilievo FALSO** — ⛔ **e la prima stesura di questa riga diceva *«quattro presi»*, smentita dalla propria enumerazione e dal diff: i Minor si contano nel `git diff`, non a memoria.** Presi: la riga di §8 del compendio, **dettata dal Passo 2**, prometteva *«esiti, **seed**, versioni e comandi»* anche per SP-7, che **non ha seed** — la sezione SP-7 di `spikes/RISULTATI.md` delimitata con `awk` e passata a `grep -ci 'seed'` rende **0**; e i due richiami brevi su AUD-032 dicevano *«l'attribuzione … **è corretta** nella §3.2»*, che in italiano si legge anche come *«è giusta»*, cioè il contrario di ciò che il richiamo esiste per dire — ora dicono *«è **stata corretta**»*. ⛔ **Falso:** il rilievo che **E17②** fosse chiusa a metà perché la §6.2 non nomina il bundle — il **dispaccio** al quale l'esecutore rispondeva prescriveva *«scegli **una** casa, non due»*, e lui ha scelto la §7; il revisore non aveva il dispaccio, e il rilievo cade. ⚖️ **Non presi, e dichiarati DUE.** ① L'URL del modello `float16` vive in **tre** occorrenze tracciate — la tabella delle versioni di SP-7 e due righe di questo piano — ma le due del piano sono **verbali datati** (*«l'URL letto il 2026-09-03»*), che è la stessa difesa della sesta casa qui sopra. ② *«nove compiti su nove»* nel disegno tende contro il **vincolo 16**, che vuole il numeratore nella sola tabella della posizione — ma il valore è **terminale** (il piano è chiuso e non può crescere) e la stringa è **dettata dal Passo 5**: si scrive qui invece di correggerla. ⛔ **E LA RI-REVISIONE DI QUESTA STESSA ONDATA HA TROVATO ALTRI TRE DIFETTI, TUTTI DENTRO QUESTA VOCE:** il *«quattro presi»* di sopra, il *«sei case»* che non contava le **quattro** occorrenze di questo piano — vere oggi, dentro un piano chiuso — e l'universale *«ogni riga col marcatore rimanda alla §6»*, che `grep -rn '⏭️' docs/` smentisce: alcune righe lo **nominano** dentro un gotcha o un comando. Corretti qui **per sottrazione** (gotcha **#76**), senza una quarta passata. 📌 **E la costante regge una volta di più:** le cifre **rimisurate sui file** hanno retto tutte, quelle **derivate da un testo** — un conteggio di rilievi, un censimento di case, un universale — hanno sbagliato tutte, che è precisamente la misura registrata chiudendo il Task 11 del Traguardo 5 |
