...
| **5** | la chiusura: la Definizione di «fatto» coi comandi; il disegno e questo piano nella §12 del compendio e in `README.md`; il puntatore della §6; la riga di questo piano in roadmap; la spunta nel disegno | uno | ✅ 2026-09-05 |
...
| **E4** | **Compito 2, Passo 6 — l'attesa «`0` celle *«aperta, dipendenza dichiarata»*» è INSODDISFACIBILE per costruzione: il testo dettato per la casa 2 cita la frase vecchia nel proprio richiamo.** Misurata dall'implementatore dopo il commit `c481394` e confermata dal coordinatore il 2026-09-05: `grep -c 'aperta, dipendenza dichiarata' "$G"` rende **1** — la citazione *«Questa cella diceva «⏳ aperta, dipendenza dichiarata»»* dentro la cella nuova — mentre la frase **come cella a sé**, `grep -c 'con un gesto . ⏳ aperta, dipendenza dichiarata . ' "$G"`, rende **1** su `8f7d078` e **0** dopo — ⚠️ **sonda corretta il 2026-09-05:** qui stava la stessa frase fra due barre verticali scritte con la barra rovesciata davanti, che copiata dal sorgente è un'alternanza vacua, **E8**. Domanda **1** del pre-controllo: la sonda contava la frase, l'attesa voleva la cella. Il testo dettato non è stato toccato; l'attesa si legge sulla sonda della cella |
...
| **E7** | **Compito 2, Passo 1 e P-4 — le case della decisione 7 sono DIECI, non nove: la voce 3 di «Le voci che questo disegno apre per il proprietario» del disegno dei gesti scrive «le decisioni 2, 7, 9, 10, 12, aperte», senza richiamo datato, in una sezione VIVA.** Trovata dalla revisione del compito 2 il 2026-09-05 (rilievo I-1) e confermata dal coordinatore: la riga non contiene né «decisione 7» né «cattura», quindi nessuno dei due censimenti di P-4 la rende — gotcha #70, che stavolta ha morso il censimento del piano e non chi lo eseguiva. La forma che la rende: `grep -n ', 7,' "$G"` → due righe, la voce 3 (viva) e la riga «Debito lasciato» della sezione «Come si riprende» del 2026-09-03, che è un verbale con la data nell'intestazione e resta (vincolo 7). Rimedio: **ondata 1 del compito 2** — un richiamo datato appeso in coda alla terza cella della voce 3, sulla forma della voce 1 della stessa sezione, senza accorciarla; la sonda `grep -c 'Richiamo del 2026-09-05: la 7 è chiusa' "$G"` → 0 prima, 1 dopo. Il messaggio del commit `c481394` dice «nove case» e resta: è un verbale |
| **E8** | **E4 — la sonda della cella, com'era scritta, è VACUA se copiata dal sorgente: la frase fra due barre verticali scritte con la barra rovesciata davanti, che nel `grep` di base è un'alternanza (vincolo 15), quindi il modello prendeva ogni riga e rendeva 645 — le righe del file — su `8f7d078` come su `c481394`.** Trovata dalla revisione del compito 2 (rilievo M-1), rimisurata dal coordinatore. E una barra nuda non si scrive in una cella di tabella: la sonda che regge non ne porta nessuna, col punto al posto della barra — `grep -c 'con un gesto . ⏳ aperta, dipendenza dichiarata . ' "$G"` → **1** su `8f7d078`, **0** su `c481394`; la direzione «deve trovare», sulla cella nuova: `grep -c 'con un gesto . ✅ \*\*presa il 2026-09-04\*\*' "$G"` → **1** su `c481394`. E4 è corretta sul posto e rimanda qui. È la trappola 14 del disegno della knowledge base nella forma speculare |
...
| **E10** | **Compito 3, Passo 3, contro P-14 e D4 — la riga di «Perché quest'ordine» dettata dal piano appende alla frase scelta dal proprietario (voce 5) una coda che la voce non contiene: «⛔ A sbarrare il 13 è la decisione registrata da AUD-004 — un ADR del proprietario, da scrivere in parallelo al 2 — non il 2», mentre P-14 diceva «nient'altro cambia della frase».** Trovata dalla revisione del compito 3 il 2026-09-05 (rilievo Minor, plan-mandated). La coda è vera e fondata — §5.4 e §5.5 punto 5 del disegno — e scioglie il nodo fra «dopo la GUI minima» e la cella «Dipende da» del 13, che non nomina il 2: per questo la riga RESTA com'è scritta (vincolo 7: si toglie il di più, non ciò che regge), e P-14 si legge come «la frase del proprietario non cambia», non come «la cella non contiene altro». Se il proprietario vuole la sua riga pulita, la coda si sposta nella cella «Dipende da» della riga 13, che già nomina AUD-004: è una sua scelta, registrata qui ⚠️ **Richiamo del 2026-09-05, dalla revisione finale dell'intero piano (rilievo M-2):** anche la TESTA della cella diverge dalla frase scelta — il piano ha scritto «**Registro delle guide (13)** dopo la GUI minima…» dove la voce 5 del disegno comincia con «13 dopo la GUI minima…», sulla forma delle altre righe della tabella («Gesti dopo…», «Backup dopo…»). Testa e coda si leggono insieme e si decidono una volta sola: del proprietario ✅ **Deciso il 2026-09-06 dal proprietario, riprendendo: A — la riga resta com'è, testa e coda insieme.** La coda è vera e scioglie il nodo; la testa segue la forma delle altre righe della tabella, che aprono col nome in grassetto. Nessun file toccato oltre a questa cella: la voce 5 del disegno resta il verbale della scelta del 2026-09-04, e la tabella del triage della terza sessione resta un verbale |
...
| **E12** | **Compito 4, Passo 3 — nel riquadro di `tracciabilita.md` la riga nuova dice «la Conoscenza è il 6 in due metà», e la riga del 2026-09-03 subito sopra usa «la sezione 6» per una sezione di quel file: lo stesso numero con due significati in due righe consecutive, mentre la Conoscenza è la sezione 3 del file.** Trovata dalla revisione del compito 4 il 2026-09-05 (rilievo M-1, Minor, plan-mandated). La frase è vera letta come sotto-progetto, e la parola «sede» nella stessa frase la disambigua; il rimedio — una parola, «il **sotto-progetto** 6» — non è stato eseguito: il proprietario ha fermato la sessione dopo l'ondata 1, e si decide in un punto solo alla revisione finale, o lo decide lui. Registrata, non presa ✅ **Rimediata il 2026-09-05 dall'ondata della revisione finale dell'intero piano** (rilievo I-2): «la Conoscenza è il sotto-progetto **6** in due metà», la parola che questa voce già proponeva. Sonda: `grep -c 'la Conoscenza è il sotto-progetto \*\*6\*\*' docs/tracciabilita.md` → 0 prima, 1 dopo |
| **E13** | **Compito 4, Passo 2, contro la §5.2 del disegno — la cella `Skills` dettata dal piano appende «se le difese di ADR-0015 si estendano alle skill: AUD-004, un ADR del proprietario prima del 13», mentre la §5.2 dettava solo «registro delle guide → 13»: la dipendenza da AUD-004 ha così una SECONDA casa viva accanto alla cella «Dipende da» della riga 13 di `roadmap.md`, scritta dal compito 3.** Trovata dalla revisione del compito 4 il 2026-09-05 (rilievo M-2, Minor, plan-mandated), misurata con `grep -rn 'difese di ADR-0015 si estend' docs --include='*.md'`: due case vive di stato, più il disegno (l'autorità), il piano e l'archivio. Stessa famiglia di E10. Quando l'ADR di AUD-004 sarà scritto, entrambe le celle andranno toccate — la forma contro cui `CLAUDE.md` mette in guardia. Le due vie: la coda resta e si accetta la seconda casa (come per E10), oppure la cella si accorcia a «registro delle guide → **13**, con provenienza e impronta all'approvazione (§1.1e …)» lasciando AUD-004 alla sola roadmap. Del proprietario, o della revisione finale: registrata, non presa ✅ **Rimediata il 2026-09-05 dall'ondata della revisione finale dell'intero piano** (rilievo I-1), per la seconda via: la coda è TOLTA dalla cella `Skills`, che torna alla lettera della §5.2 del disegno, e la dipendenza da AUD-004 resta nella sola cella «Dipende da» della riga 13 di `roadmap.md` — `CLAUDE.md`: un puntatore che vive in più documenti si toglie. Sonde: `grep -c 'AUD-004' docs/tracciabilita.md` → 1 prima, 0 dopo; `grep -c '→ \*\*13\*\*' docs/tracciabilita.md` → 3 invariato |
| **E14** | **Compito 5, Passo 1, condizione 4 — l'attesa «`grep -c 'aperta, dipendenza dichiarata' docs/superpowers/specs/2026-09-03-riconoscimento-gesti-design.md` → `0`» è INSODDISFACIBILE per costruzione, per la ragione di E4: la cella nuova della riga 7 cita la frase vecchia nel proprio richiamo, quindi il comando rende **1** da `c481394` in avanti.** Trovata dal pre-controllo del compito 5, il 2026-09-05, rilanciando il comando su `5e4c75a` prima di dispacciare: rende **1**. Domanda **1** del pre-controllo — la sonda conta la frase, la condizione vuole la cella — e il disegno della knowledge base, che è l'autorità, dice solo «`grep` sulla frase» senza fissarla: cede la sonda del piano, non il testo. La condizione 4 si legge sulle due sonde di E8, senza barre, con `$G` il disegno dei gesti: `grep -c 'con un gesto . ⏳ aperta, dipendenza dichiarata . ' "$G"` → **0**, e la direzione «deve trovare» `grep -c 'con un gesto . ✅ \*\*presa il 2026-09-04\*\*' "$G"` → **1**; il comando del Passo 1 resta scritto com'è, e la sua resa attesa è **1**. Scritta prima di dispacciare il compito |
