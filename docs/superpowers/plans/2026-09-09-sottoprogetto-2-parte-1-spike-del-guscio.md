...
✅ **In esecuzione dal 2026-09-10: i compiti 1–8 sono CHIUSI** — dieci commit di compito in tre sessioni più il compito 7 nella quarta, l'errata E1–E14 (E11 dal compito 6, E12 ed E13 alla sesta chiusura: la cadenza del campionatore, il comando `\\$f`, i segnaposto nelle celle «Comando» — nessuna con un rimedio al codice; E14 dal compito 7, sotto `file://` dockview rifiuta la finestra a parte, senza rimedio in questo piano; il rimedio di E9/E10 eseguito il 2026-09-10 come commit d'errata a sé, la via A su delega del proprietario) — il compito 7 eseguito il 2026-09-10 dal coordinatore col proprietario allo schermo (regola 7 di «Come si esegue»): `dockview` resta otto su otto, il guscio è **Electron** (A, decisione del proprietario), Q3 rifiutata nei due gusci con le cause lette — e ✅ **il compito 8, la chiusura, ESEGUITO il 2026-09-10** nella quinta sessione, con l'errata E15–E19 dal suo pre-controllo: **la parte 1 è ESEGUITA**, ADR-0029 `Accepted`, e ciò che viene dopo sta nella §6 del compendio («Dopo il compito 8»). Il pre-controllo delle quattro
...
| **6** | le misure M1–M5 e Q1–Q2 sui due gusci installati, con gli script; la sezione SP-8 di `spikes/RISULTATI.md` coi numeri e le righe del proprietario dichiarate ⏳ | uno | ✅ 2026-09-10 |
| **7** | col **proprietario**: le otto mosse nel browser, Q3 e Q4 nei due gusci, la CPU con la chat nascosta e la VRAM a pagina intera, la domanda A/B sul guscio; l'esito completo, la riga SP-8 chiusa | uno | ✅ 2026-09-10 |
...
| **E14** | **P-4 e il compito 4 — sotto `file://` dockview 8.2.0 rifiuta la finestra a parte a prescindere dall'URL, e `popoutUrl: 'popout.html'` relativo non basta.** Misurato il 2026-09-10 al compito 7 dalla console del renderer dell'app Electron installata (script sul Chrome DevTools Protocol, sola lettura): `dockview: popout URL must be same-origin http(s); got: popout.html`, prima di qualunque `window.open`; il guscio ammette la finestra figlia (`setWindowOpenHandler` → `allow`) e `dist/popout.html` esiste nel pacchetto. P-4 aveva letto il default `/popout.html` e dedotto che un percorso relativo sotto `file://` bastasse (domanda 1): il controllo dell'origine non era stato letto. **Nessun rimedio in questo piano:** Q3 di Electron è `REFUSED` nell'esito con la causa (O7), ADR-0029 al compito 8 lo scrive come fatto per guscio; il rimedio — servire la build da un'origine http(s), un server locale o un handler di protocollo — è **dedotto** e appartiene al guscio della SPA nella parte 2, dove si misura. In Tauri la pagina vive su `http://tauri.localhost/` e il controllo passa: lì il rifiuto è di `window.open`, che WebView2 rende null (O7) |
| **E15** | **Compito 8, Passo 2 — il blocco della *Decision* dice «la macchina è quella di ADR-0002», e la riga M5 del modello porta un numero per slot: entrambi FALSI da E9 ed E10.** La macchina delle misure è il portatile — `Intel(R) Core(TM) i7-14700HX`, `Intel(R) UHD Graphics`, `NVIDIA GeForce RTX 4060 Laptop GPU` — con la webview sull'integrata (`api=WebGPU:intel/gen-12lp`), come la riga «La macchina» dell'esito scrive; e M5 è **due** letture per corsa, dedicata / condivisa (`0 / n`), perché la dedicata è zero per costruzione sull'integrata. Trovata dal coordinatore il 2026-09-10 al pre-controllo del compito 8, rileggendo il modello contro l'esito (riga 5 del pre-controllo di `CLAUDE.md`: il compito è del 2026-09-09, E9 ed E10 del 2026-09-10). **Correzione:** il blocco nomina la macchina dell'esito e dice che **non** è quella di ADR-0002 — i numeri assoluti di M3 e M5 non sono quelli della RTX 5080, il confronto fra i gusci regge — e la riga M5 porta le coppie «dedicata / condivisa» con la condivisa dichiarata **proxy** (E10); il testo corretto è nel dispaccio del compito. Costo se sbagliato: nessuno, è una lettura |
| **E16** | **Compito 8, Passo 2 — i modelli di Q3 e di M4, riletti contro l'esito come la settima chiusura prescrive, non reggono alla lettera.** Q3 non è «sì, oppure no»: è `REFUSED` in **entrambi** i gusci per cause diverse (O7, E14) — Electron perché `dockview` rifiuta sotto `file://` prima di `window.open`, con un rimedio **dedotto** per la parte 2; Tauri perché `window.open` rende null in WebView2 — e il protocollo dice che un no in un guscio non è un no a `dockview`; M4 `non passa` su entrambi anche con la chat nascosta, ed è la scena `three` senza tetto di fps, che a riposo sta già vicino o sopra la soglia (O2): un fatto per la **SPA**, non un discriminante fra i gusci — la settima chiusura lo vuole in ADR-0029 accanto a P3, e il modello non ha dove metterlo. Trovata dal coordinatore il 2026-09-10 al pre-controllo. **Correzione:** le celle Q3 portano `no` con la causa per guscio; un capoverso sotto la tabella porta il fatto di P3 e il proxy di M5; il blocco `Negative (accettate)` di Electron guadagna la voce sulla finestra a parte sotto `file://`, col rimedio dedotto e da misurare nella parte 2; Q4 sta nel modello (`'auto'` Electron, `'pointer'` Tauri). Il testo nel dispaccio. Costo: nessuno |
| **E17** | **Compito 8, Passo 5 — mancano tre richiami: due che E1 ed E2 avevano assegnato al compito 8 (più la decisione 10 della quinta chiusura), e uno che il pre-controllo trova.** (a) La §4 della stella polare: la clausola «EQUAL / DIFFERENT» della mossa 7 nel protocollo (E1) e il confronto sul JSON canonico (E4) — il richiamo del Passo 5 non li nomina. (b) `dockview-core` 8.2.0 che non spedisce il CSS (E2) e l'emettitore di `gui-ipc` riusato com'è, chiavi italiane comprese (decisione 10): E2 e la settima chiusura li mandano alla «§2 della stella polare», che non nomina né `dockview-core` né `gui-ipc` (`grep -n` il 2026-09-10) — la casa è la **§2 del disegno del 2**, le righe «Vue 3 con `vite` e `dockview-core`» e «l'emettitore di `spikes/gui-ipc/` com'è», dove il richiamo del Passo 5 già va, e la §4 della stella polare lo dice («la §2 del 2 riceve un richiamo datato»). (c) La riga «**Assunto:** che `dockview.toJSON()` si rimetta com'era — lo misura lo spike di accettazione» della §2 della stella polare è **misurata** dalla mossa 7 — canonico EQUAL, grezzo DIFFERENT per l'ordine delle chiavi, +2 px per gruppo galleggiante, i minimi su un viewport stretto (E4) — e non ha un richiamo. Trovata dal coordinatore il 2026-09-10 al pre-controllo. **Correzione:** i tre richiami nel dispaccio, coi *Trova* presi dal file; la mappa dei file non cambia, i due disegni sono già del compito 8. Costo: nessuno |
| **E18** | **Compito 8, Passi 1 e 9 — l'`awk` delle tabelle spezzate non può rendere «lo stesso output» alla lettera: i numeri di riga si muovono con le righe aggiunte o tolte sopra di loro.** Il punto 2 della §6 del compendio passa da quattro righe a una e la tabella «Chiuso» ne guadagna una (883 → 881, **dedotto**); `HANDOFF.md` guadagna una riga a 155–156 (1155 → 1156, **dedotto**); `riferimenti.md` non si muove, perché la sezione SP-8 entra a 2458, dopo le quattro righe. Trovata dal coordinatore il 2026-09-10 al pre-controllo — la specie della sonda sbagliata, domanda 1. **Correzione:** al Passo 9 si pretende **lo stesso insieme** — sei righe, gli stessi file, nessuna nuova sui file toccati — e i numeri si annotano com'escono. Costo: nessuno |
| **E19** | **Compito 8, Passo 9 e criterio di chiusura — «le due app si disinstallano e i CSV si cancellano» PRIMA del commit contraddice la settima chiusura, che li vuole DOPO la revisione del compito 8** (il revisore può rileggere i CSV e le prove in `$HOME/sp8-measure/`). Trovata dal coordinatore il 2026-09-10 al pre-controllo. **Correzione:** il compito 8 committa coi documenti e lascia le app installate — il messaggio del commit lo dice al posto di «le due app disinstallate» — e la disinstallazione, la cancellazione della cartella e la loro verifica le fa il coordinatore alla chiusura della sessione, dopo la revisione, registrandole nell'ottava chiusura del diario; la casella del criterio si chiude lì. ⚠️ **Dedotto e da misurare:** un disinstallatore NSIS lanciato con `/S` si copia in `%TEMP%` e **torna subito**, quindi `Start-Process -Wait` non aspetta la fine — dopo il lancio si attende che la cartella sparisca; `sp8-electron-updater` la lascia l'installatore e si toglie a mano. Costo: le app restano sulla macchina fino alla chiusura |
| **E20** | **Compito 8, Passo 8, condizione 3 — la sonda `grep -c 'move 7: EQUAL\|move 7: DIFFERENT'` → `1` è invecchiata con E4, e rende `0`.** Dal commit del compito 2 la mossa 7 si misura sul JSON canonico, e la riga vera dell'esito è `move 7: canonical EQUAL; raw DIFFERENT at byte 1798 (3052 vs 3052 bytes)`, che non contiene la sottostringa `move 7: EQUAL`. Misurato il 2026-09-10 dall'implementatore del compito 8, che ha eseguito la sonda dettata, riportato lo zero e verificato la sostanza con la sonda sul canonico — `grep -c 'move 7: canonical EQUAL\|move 7: canonical DIFFERENT'` → `2` (la riga 7 delle mosse e O6); la riga ufficiale della mossa 7 è `canonical EQUAL`, la condizione regge; confermato dal revisore. **Correzione:** la sonda della condizione 3 è quella sul canonico; il brief resta com'era, è eseguito, e questa voce lo dice. Costo: nessuno |
...
✅ **Misurato il 2026-09-10, compito 7:** il proxy di `vite` passa il flusso `text/event-stream` — la tessera Mano ha scritto `relay connected` e la mano si è disegnata; il ripiego nel relay non è servito, `spikes/gesti/` intatto.
...
✅ **Misurato il 2026-09-10, compito 7:** `dockview` in `'pointer'` **segue** il puntatore sintetico — 15 pinze sulla presa grande, la tessera si è staccata e spostata con la mano (sezione SP-8 di `spikes/RISULTATI.md`, mossa 8): nessun `InvalidPointerId`, il no tecnico non c'è stato.
...
### L'ottava chiusura — 2026-09-11: il compito 8 ESEGUITO (il 10) e revisionato (l'11) — LA PARTE 1 È ESEGUITA, le app disinstallate, le misure cancellate — si riprende dalla PARTE 2, in una sessione NUOVA, dalla §6 del compendio

⛔ **DA SAPERE SUBITO.** Niente è a metà: albero pulito, nessuno stash, nessuna operazione git in corso, nessun server acceso, nessun
processo dei gusci vivo, nessun codice di prodotto toccato. **Questo piano è ESEGUITO per intero** — i compiti 1–8 ✅ nella tabella
della posizione — e non ha un compito successivo: ciò che viene dopo sta nella §6 del compendio, la **parte 2** del piano del 2
(`superpowers:writing-plans`, i pezzi 2–9 della §3 della stella polare, con lo stesso pre-controllo). Le due app **sono
disinstallate** e `$HOME/sp8-measure/` **è cancellata** alla chiusura, dopo la revisione (E19): misurato l'11 mattina con `uninstall-sp8.ps1` (scratchpad) — `Uninstall sp8-electron.exe /S` con `Start-Process -Wait` è tornato dopo ~18 s con la cartella già sparita, `uninstall.exe /S` di Tauri dopo ~3 s, idem; la cache `%LOCALAPPDATA%\sp8-electron-updater` (una copia dell'installatore) è rimasta ed è stata tolta a mano; nessuna chiave `HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall` con `sp8`, nessun processo; `$HOME/sp8-measure/` tolta coi suoi sedici file — dieci CSV, tre prove, tre attrezzi. Gli installatori
restano in `spikes/gui-shell/electron/out/` e in `tauri/src-tauri/target/release/bundle/nsis/`, ignorati da git e rifacibili con
`npm run dist` e `tauri build`. Il commit `5dc9c6f` porta ancora il trailer `Co-Authored-By` (quarta chiusura, decisione 4: resta
...
| I commit di questa sessione | `git log --oneline 592ebc3..HEAD` — `97c44c3` (l'errata E15–E19, prima del dispaccio), `34ec98f` (il compito 8), poi questa chiusura |
| Codice di prodotto, cancello, CI, gli spike | **non toccati**: `git diff --stat 5ad4634..HEAD -- crates/ scripts/ .github/ Cargo.lock Cargo.toml rust-toolchain.toml spikes/gesti/ spikes/gui-ipc/` non rende nulla; e in questa sessione nemmeno `spikes/gui-shell/`: `git diff --stat 592ebc3..HEAD -- spikes/gui-shell/` non rende nulla |
| La posizione e l'errata | i compiti **1–8** a `✅ 2026-09-10`: `grep -c '^\| \*\*[1-8]\*\* \|.*✅ 2026-09-10' <piano>` → `8`; l'errata **E1–E20** — `grep -c '^\| \*\*E[0-9]*\*\* \|' <piano>` → `20` |
| ADR-0029 | `Accepted`: `grep -c '^- \*\*Status:\*\* Accepted' docs/adr/0029-guscio-della-gui.md` → `1`; `bash scripts/check-docs.sh` stampa `  (none)` sotto `== ADR still in Proposed ==`; la tabella M1–Q4 nella *Decision*: l'`awk` della condizione 2 del Passo 8 del compito 8 → `9` |
| Le case della decisione aperta | il `grep` di `Proposed` filtrato del Passo 9 del compito 8 → una riga sola, `docs/audit-2026-08-27.md:29` (il verbale di come l'audit fu condotto) |
| Il prossimo passo | `grep -c '⏭️' docs/COMPENDIO.md` → `3`; il puntatore della §6 dice «LA PARTE 2», e la §6 è l'unica casa |
| Cancello | `bash scripts/gate.sh` → `GATE GREEN` (log datati nello scratchpad: `gate-2026-09-10-apertura-s8.log`, `gate-2026-09-10-errata-e15-e19.log`, `gate-task8-2041.log` dell'implementatore, `gate-review-task8-0752.log` del revisore, `gate-2026-09-11-chiusura-s8.log` di questa chiusura); `bash scripts/check-docs.sh` → `OK` |
| Fine-riga | il piano **LF** (`tr -cd '\r' < <piano> \| wc -c` → `0`); i nove CRLF con CR = righe, elencati nel rapporto del compito 8 (`task-8-report.md`) |
| Margine del compendio | il comando del vincolo 11: `11030` dopo il compito 8 — l'intestazione corta ha liberato più di quanto le righe nuove hanno consumato (P-17 e P-22 confermate) |
| Le app e le misure | `powershell -NoProfile -Command "Get-ChildItem \$env:LOCALAPPDATA\Programs,\$env:LOCALAPPDATA -Directory -Filter 'sp8-*'"` → niente; `ls "$HOME/sp8-measure"` → la cartella non esiste |
| Il registro di esecuzione | `.superpowers/sdd/2026-09-09-sottoprogetto-2-parte-1-spike-del-guscio/progress.md` su **questa** macchina, ignorato da git, con la riga della sessione 5; `task-8-brief.md`, `task-8-dispatch.md`, `task-8-report.md`, `task-8-review-dispatch.md`, `task-8-review-report.md` |

#### Le decisioni prese eseguendo, nell'ordine (seguono le trentaquattro della settima chiusura)

| # | Decisione | Perché | Costo se sbagliata |
|---|---|---|---|
| 35 | l'errata E15–E19 committata **da sola** (`97c44c3`) prima del dispaccio, col cancello e `check-docs.sh` | il Passo 1 del compito pretende `git status --porcelain` vuoto dopo la corsa dello spike: un piano modificato e non committato lo avrebbe reso falso, o avrebbe voluto un filtro | un commit in più |
| 36 | i sette gotcha 112–118 **dettati dal coordinatore** nel dispaccio, non lasciati all'implementatore; il criterio del Passo 7 applicato alle voci d'errata che hanno fatto sbagliare un passo (E2/E3/E14, E9/E10, E11, E13, E12, E7/E8) e alla trappola della finestra coperta (O1) | la settima chiusura li mandava a `HANDOFF.md`; una riga di gotcha è densa e nella forma della tabella, e un implementatore fresco non ha il contesto per scriverla | sette righe da togliere o accorciare, se il proprietario le trova ridondanti |
| 37 | la casa del richiamo su `dockview` (CSS, E2) e sul contratto di `gui-ipc` (decisione 10) è la **§2 del disegno del 2**, non la «§2 della stella polare» che E2 e la settima chiusura nominavano | misurato: la §2 della stella polare non nomina né `dockview-core` né `gui-ipc`; la §4 della stella polare dice «la §2 del 2 riceve un richiamo datato» (E17b) | nessuno: è una lettura |
| 38 | l'assunto della §2 della stella polare — «che `dockview.toJSON()` si rimetta com'era» — riceve il richiamo **misurato** dalla mossa 7 (E17c), con la conseguenza «chi confronta due disposizioni confronta il canonico, non i byte» | la regola di `CLAUDE.md`: uno schema è una verifica e corregge ciò che esiste, col richiamo datato, mai in silenzio | il proprietario può ribaltare la conseguenza; il fatto misurato resta |
| 39 | la disinstallazione e la cancellazione delle misure **dopo** la revisione, alla chiusura (E19), e il compito 8 committato con le app installate | la settima chiusura lo aveva deciso; il revisore poteva rileggere i CSV e le prove | nessuno |
| 40 | E20 scritta dal rapporto dell'implementatore: la sonda della condizione 3 del Passo 8 (`move 7: EQUAL`) è invecchiata con E4; la sonda giusta è sul canonico | «una divergenza è una voce d'errata prima di essere un rimedio»; l'implementatore ha eseguito la sonda dettata, riportato lo zero e verificato la sostanza con una seconda sonda dichiarata — il comportamento giusto | nessuno |
| 41 | il rilievo **Important** del revisore — un frammento duplicato nella riga delle decisioni deliberate dell'audit — corretto dal coordinatore **a mano**, senza ri-revisione | è prosa: nessun fatto né comando cambia (regola 6 di «Come si esegue»), e il revisore stesso lo dice «non un problema di sostanza» | nessuno: la riga è verificata col `grep` e con CR = righe prima del commit di chiusura |
| 42 | il secondo revisore dispacciato **da zero** dopo lo stallo del primo, senza cercare di riprenderlo o di riusare il suo log | `SendMessage` non esiste (riconfermato con `ToolSearch`); il primo aveva lasciato solo un log del cancello — verde, ma non una prova sua — e nessun rapporto | una revisione rifatta, ~18 minuti |
| 43 | il compendio **non** si tocca in questa chiusura: il puntatore della §6 dice già «la parte 2» dal compito 8, e nulla di merito è cambiato | «cambio del prossimo passo» è l'unico gancio della §13, e non c'è; un tocco senza merito rimetterebbe in gioco la riga «Aggiornato il» per niente (gotcha #31) | nessuno |

#### Che cosa il revisore ha trovato, e dove è finito

| Compito | Rilievo | Dove |
|---|---|---|
| 8 | **Approvato con rilievi** — revisore `sonnet` in sola lettura, `task-8-review-report.md`, 2026-09-11 (il **primo** revisore, in background, si è fermato nella sospensione notturna della macchina — «no progress for 600s» — senza scrivere nulla; il secondo ha rifatto tutto): le sonde del Passo 9 e i blocchi della Definizione di «fatto» rilanciati e uguali all'atteso, ADR-0029 letto contro 0027, 0030 e 0033 senza contraddizioni e **cifra per cifra** contro l'esito, i gotcha 112–118 uguali al dispaccio byte per byte, i cinque richiami uguali ai testi dettati, l'archivio parola per parola, `GATE GREEN` rifatto (`gate-review-task8-0752.log`) e `check-docs.sh` → `OK` | — |
| 8 | **Important**: `docs/audit-2026-08-27.md`, la riga di «Le decisioni deliberate», portava «`#[ignore]`, il» **due volte** — la sostituzione del Passo 4 aveva ricopiato la coda del *Trova* dopo il testo nuovo | corretto dal coordinatore a mano in questa chiusura (Python con `newline=""`, CRLF conservato, CR = righe = 1885): è prosa, nessun fatto né comando — decisione 41 |
| 8 | la divergenza dell'implementatore sulla sonda della condizione 3 del Passo 8 **confermata** e giudicata senza conseguenze sulla sostanza; il revisore ha proposto il testo di E20 | E20 scritta in questa chiusura, nella forma della sua proposta |
| 8 | dichiarato dal revisore: tre file temporanei creati per sbaglio nella radice del repository da un percorso relativo in uno script, tolti nello stesso turno | nessuna traccia: `git status --porcelain` vuoto, riverificato dal coordinatore prima del commit di chiusura |

#### Le trappole di questa sessione — istruzioni, non aneddoti

- ⛔ **l'estrazione «per intestazione» del compito 8 con `awk '/^## Compito 8:/{f=1} /^## L.intestazione del compendio/{exit} f'`, come la settima chiusura diceva, TAGLIA il compito a metà**: quell'intestazione sta **dentro** un blocco di codice del Passo 3 (il modello del blocco d'archivio), e l'`awk` non sa che è una recinzione. Si esce sull'intestazione **vera** successiva — `/^## Dopo il compito 8/` — e si conta: 333 righe. Vale per ogni compito che porta modelli con `##` nei blocchi di codice.
- **i *Trova* di un compito si verificano TUTTI prima del dispaccio con un solo script Python** (`check_trova.py`: una lista `(file, etichetta, testo)`, `text.count(s)`, `1` atteso): trentanove verificati in un colpo il 2026-09-10, zero problemi — e il dispaccio può dire «tutti unici, verificato» invece di far scoprire un doppione all'implementatore.
- **la tabella dell'errata ha DUE colonne** (`| # | Voce |`): una riga nuova ha tre barre verticali, non quattro; un'asserzione scritta a memoria è scattata prima di scrivere — bene — ma era sbagliata lei.
- **una sessione aperta NELLA cartella del repo si comporta come le prime tre**: il cwd persiste, lo scratchpad è corto (~130 caratteri), `python percorso.py` funziona, `CLAUDE.md` e la memoria si caricano da soli — la settima chiusura aveva ragione a chiederlo.
- **`git diff` e `git add` sul piano LF stampano «LF will be replaced by CRLF the next time Git touches it»**: è `core.autocrlf=true`, un avviso su un checkout futuro, non un cambiamento del file — `git diff --cached --stat` e `tr -cd '\r'` lo provano; si registra, non si «corregge».
- **il modello di un ADR chiuso si rilegge contro le voci d'errata NATE DOPO il compito** (riga 5 del pre-controllo): E9/E10 avevano riscritto la macchina e M5, e il compito 8 — scritto il 2026-09-09 — diceva ancora «la macchina è quella di ADR-0002». Cinque difetti su un compito di soli documenti, come per tutti i compiti precedenti.
- **la deduzione di E19 sul disinstallatore NSIS era sbagliata nel verso comodo**: con `/S` e `Start-Process -Wait` il disinstallatore di `electron-builder` è tornato dopo ~18 s con la cartella già sparita, quello di Tauri dopo ~3 s; l'attesa a sondaggio non ha dovuto aspettare nulla. Ciò che resta davvero è la cache `%LOCALAPPDATA%\sp8-electron-updater`, che nessun disinstallatore toglie: si toglie a mano e si verifica con `Get-ChildItem -Directory -Filter 'sp8-*'` su `Programs` e su `%LOCALAPPDATA%`, più le chiavi `HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall`.
- ⛔ **un subagente in background muore se la macchina va in sospensione** («Agent stalled: no progress for 600s»): il cancello che aveva lanciato in background è finito verde la mattina dopo, ma il rapporto non è mai stato scritto e l'agente non si riprende — `SendMessage` non esiste in questo harness. Una revisione lunga si lascia partire solo con la macchina che resta sveglia, o si ridispaccia da zero al risveglio.
- **`Start-Process -Wait` e i disinstallatori NSIS**: vedi sopra — non serve un ciclo d'attesa, serve la verifica delle cartelle e delle chiavi dopo.
...
2. La lettura obbligatoria di `CLAUDE.md`; la **§6 del compendio** dice il passo: la **parte 2** del piano del 2. ⛔ **RICHIAMO DEL 2026-09-11 — questo punto è stato RIPREZZATO leggendo il piano, e chiedeva PIÙ del necessario** (riga 8 del pre-controllo di `CLAUDE.md`): diceva *«di questo piano si leggono la testa, i vincoli, la posizione (tutta ✅), l'errata E1–E20 e questo diario»*, cioè **54 741** token di un piano **eseguito**. Di questo piano si leggono **tre pezzi soli — 9 288 token, misurati il 2026-09-11**: *«Le decisioni prese da questo piano»* (D1–D14, di cui la parte 2 eredita **D13** e **D14**), *«Le voci aperte che questo piano SA, e non chiude»* (le voci si **sanno** prima di scrivere) e **questa ottava chiusura**. ⚠️ **Il resto non è perso: è vivo altrove, ed è LÌ che si legge** — l'esito di SP-8 in **ADR-0029** e nella sezione SP-8 di `spikes/RISULTATI.md`; le lezioni durevoli dell'errata **E1–E20** nei gotcha **#112–#118** di `HANDOFF.md` e nei richiami datati dei due disegni (E17) — verificato voce per voce il 2026-09-11. La testa, i vincoli, *«come si esegue»*, il pre-controllo **P-1…P-25**, la mappa dei file e l'errata riguardano l'**esecuzione** di SP-8, che è finita: si aprono **a domanda**, non di lista.
3. `superpowers:writing-plans` per la parte 2, dalla **§10 del disegno del 2** («Come si riprende», punto 8) e dalla §3 della stella polare (i pezzi 2–9), coi due disegni letti **per intero** e coi numeri di ADR-0029 in mano: chi decodifica (Q1: Node, `bincode-ts`), il guscio Electron che serve la build da un'origine http(s) per la finestra a parte (Q3, dedotto — si misura lì), P3 che non passa per la scena senza tetto di fps (un tetto, o il rendering a richiesta, da misurare), M5 proxy sull'integrata; X-1 (la CI anche su Windows) e X-3 (`cargo audit`, `npm audit`) come compiti della parte 2 (D13); le righe nella §12 del compendio, in `README.md`, nella roadmap e in tracciabilità entrano con essa (D14). Il pre-controllo delle quattro domande **nella sessione che scrive il piano**, e l'esecuzione in sessioni nuove.
4. Prima di scrivere: le voci aperte **si sanno** (`CLAUDE.md`, `superpowers:writing-plans`) — la §9 del disegno del 2, le registrate della stella polare, le voci senza numero AUD dell'audit, le tabelle dei Traguardi 5 e 6 di `porta-di-qualita.md`.
5. Alla chiusura di ogni sessione: il diario del piano nuovo, la memoria dell'agente, `session-handoff`.


### ⛔ Le chiusure 1–7, ARCHIVIATE il 2026-09-11 — la cronaca di uno spike ESEGUITO

Qui stavano, **parola per parola**, le prime sette chiusure di questo diario. Stanno in
[`archivio/diario-spike-guscio-storico.md`](../../archivio/diario-spike-guscio-storico.md), coi
loro byte, e si aprono **una per volta**, con una domanda storica in mano — non per farsi
un'idea.

⚠️ **Non è un taglio, è uno spostamento, e la ragione è misurata il 2026-09-11:** il diario
intero pesava **24 650** token (`cl100k_base`) sulla lettura di **ogni** ripresa, per raccontare
sessioni chiuse. Il comando che rifà la misura, e i suoi limiti, stanno nella sezione
*«Sfoltimento del compendio»* di [`riferimenti.md`](../../riferimenti.md); la lezione è il gotcha
**#119** di [`HANDOFF.md`](../../HANDOFF.md).
