...
| `HEAD` = `baf3cde`; D = 74; il piano di 20 129 righe | `HEAD` = `b3422bf` (`git rev-parse --short HEAD`); `grep -c '^| \*\*D[0-9]' <piano>` → **89**; `wc -l <piano>` dà le righe di oggi. P **133** dal 2026-09-16 (decisione 94 → B: P-117…P-133, una per compito rivisto, che rimandano ai rapporti e a `ledger.md`; erano **116** a `b3422bf`), i compiti **17**, l'errata **vuota**, nessun compito eseguito, `gui/` non esiste, il codice di prodotto identico a `42b50d8` (il comando della §2 resta vuoto) |
