...
✅ **Le correzioni sono applicate, il 2026-09-23**, nel commit che porta questa riga. Questo registro resta la **casa** dei
rilievi e del loro stato (⬜ da applicare, ✅ applicato); come sono state applicate lo dice la sezione in fondo.
...
| **R2-3** e **R3-6**, i PNG delle prove rosse | il **coordinatore** | `screenshotFailures: false` nel progetto `browser` del compito 2, con la ragione accanto (il cancello non guarda immagini, e il nome di un file supera i 260 caratteri di Windows); i passi delle due direzioni finiscono con `git status --porcelain` vuoto. ⚠️ Da rimisurare applicandolo: che anche `gui/.vitest-attachments/` non nasca più; se nasce, una riga in `.gitignore`. ✅ **Rimisurato il 2026-09-23, nel Chrome installato: non nasce** — dopo un rosso nel browser `git status --porcelain` è quello di prima, senza `__screenshots__` né `gui/.vitest-attachments/`; nessuna riga in `.gitignore`. E il *«vuoto»* di questa riga è diventato *«quello di prima»* (A-1) |
...
| R3-22 | Minore | disegno (c), riga 6 | `DockviewTheme` ha undici campi, non sette | la riga 6 è **già corretta** (`dndOverlayBorder`); il richiamo nel disegno è scritto | ✅ |
...
## L'applicazione — 2026-09-23

✅ **Le correzioni sono scritte**, nel commit che porta questa sezione: **73**, ad ancore uniche asserite tutte prima di
scrivere, nel piano, nel disegno e nella tavola — tre file LF, e LF dopo (`tr -cd '\r'` rende 0 su ciascuno). Due sessioni,
tutte e due sulla macchina dell'account `Jays`:

- la **prima** le ha provate in una cartella di prova sua, `C:\Users\Jays\AppData\Local\Temp\pds`, col suo `git`: la base a
  `82e64a2`, i compiti 1–5 applicati dal testo del piano, un commit per compito, poi uno per ciascuna correzione che tocca il
  codice o le prove. Le ha raccolte in uno script, `tools\ledger\apply_ledger.py` di quella cartella, e non l'ha lanciato:
  Chrome non c'era;
- la **seconda**, col Chrome 154 installato, ha fatto girare nel browser quelle che lo aspettavano — R2-2, R2-3, R2-4, R2-6,
  R2-7, R3-1…R3-4, R3-7, R3-8, R3-24 — e **ogni** direzione rossa del passo 5 del compito 2 e del passo 7 del compito 4, una
  alla volta, con la copia salvata, `cmp` e `git status --porcelain` confrontato con prima dopo ciascuna; ha corretto due
  righe dello script (A-4, A-5) e lo ha lanciato.
...
| **A-4** | una riga nuova nel passo 5 del compito 2: la terza asserzione di R2-4 ha la sua direzione rossa — `--font-display` col solo ripiego, `"Bahnschrift", system-ui, sans-serif` → `expected 0 to be greater than 0.5` alla riga che confronta il token con la sua catena senza Barlow, mentre **tutte** le asserzioni prima restano verdi, le due direzioni delle larghezze comprese; e `board.test.ts` rosso, verde dopo il ritorno | la correzione proposta aggiungeva l'asserzione senza la sua violazione, e il vincolo 11 vuole ogni prova nelle due direzioni: la violazione è proprio il caso che R2-4 descrive, il token che disegna col ripiego |
| **A-5** | la riga della striscia nel passo 7 del compito 4 dice **due** rossi: `fits`, *«sticks out: base-button of kit-strip»*, e la guardia `axe` di R3-7, `expected 1 to be +0`, nei due temi | il pulsante «moduli» che sborda è in parte coperto, e `axe` ne lascia il contrasto fra gli `incomplete` — `elmPartiallyObscured`, letto con una sonda di diagnosi; il rapporto di R3-2 non poteva vederlo, perché la guardia di R3-7 non c'era ancora. È la direzione rossa di quella guardia, che la correzione proposta non dava |
| **Chrome 154** | le cifre che i commenti delle prove citano, misurate dal rapporto su Chrome 153 sull'altra macchina, qui sono **identiche**: Barlow 67,33 e 108,11 px normali, 100,42 tabulari; Bahnschrift 63,84 e 102,81, e 102,19; fra le marche c'è `"Google Chrome"`, accanto a `"Chromium"`; `color-contrast` giudica 79 nodi sulla pagina kit e non ne lascia nessuno incompleto; la finestra delle prove è 1440 × 900 | un'evidenza scritta prima della misura è un'ipotesi (`CLAUDE.md`) |
| **gli altri rossi** | come li scrive lo script: il canale inesistente → `Unsupported chromium channel "chrome-that-does-not-exist"`, uscita 1; Barlow 300 tolto → `expected 3 to be greater than or equal to 4`; il movimento → `--duration-fast: expected '110ms' to be '0ms'`, e `board.test.ts` verde dopo il ritorno; l'alto contrasto → `expected false to be true`; le sei righe del compito 4 oltre la striscia, ciascuna col messaggio della sua riga, nei due temi; R3-8 → la pagina e la finestra del tema scuro, `board.test.ts` e `contrast.test.ts`; R3-24 col `Settings.vue` di prima del compito 5 → `expected [] to have a length of 2 but got +0` | — |

⚠️ **Le cartelle di prova.** Quelle dei revisori, sull'altra macchina — `C:\Users\zagor\AppData\Local\Temp\probe-ds1`,
`probe-ds2`, `probe-ds3` —, adesso si cancellano, come vuole il punto 3 di *«Come si applica»*: lo fa chi apre una sessione
là. Quella di questa macchina, `pds`, **resta** finché i compiti 6–9 non sono scritti: porta i compiti 1–5 con le correzioni
di codice e di prova, non quelle di sola prosa — il commento di `watchTheme` vi dice ancora *«first paint»* (R1-13) —, ed è il
banco dove provare i compiti nuovi mentre si scrivono. Non è una fonte: la fonte è il piano, e la cartella si rifà da lì.
