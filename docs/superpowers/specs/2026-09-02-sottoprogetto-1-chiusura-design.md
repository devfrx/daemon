...
| `V24` | ⚠️ | test a esempi: il picco di VRAM **e** i permessi attivi si ricavano **rileggendo il giornale** | i permessi rileggono davvero: `is_granted` in `crates/kernel/src/permission.rs` chiama `journal.replay()`, provato da `nothing_is_granted_on_an_empty_journal` in `crates/kernel/tests/permission_triple.rs`. ⛔ Il picco no: `grep -rn 'VramPeak' crates/ --include=*.rs` lo trova in `crates/kernel/src/wire/worker.rs` e nei test del filo, e `a_vram_peak_survives_the_round_trip` è un giro di codifica e decodifica. I tre `journal.replay()` del kernel stanno in `degradation.rs`, `permission.rs` e `reconcile.rs`, e nessuno legge il picco | ❌ sulla metà del picco — **stesso criterio di `V8`**: la cosa esiste ed è provata, ma non è quella che la cella nomina |
