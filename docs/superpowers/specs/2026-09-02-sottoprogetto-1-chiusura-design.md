...
| | Perché |
|---|---|
| non tocca la spec | la §0.7 è una definizione, non uno stato. Lo stato ha già le sue case, e la spec è del proprietario (vincolo globale 7). ⛔ **RICHIAMO DEL 2026-09-03: questa riga è stata SMENTITA.** Il proprietario ha **sospeso** il vincolo globale 1 per il solo **compito 3bis** e per le sole **§8.3** e **§8.4**, che sono state toccate — voce **E10** dell'errata del [piano](../plans/2026-09-02-sottoprogetto-1-chiusura.md). Il perimetro resta questo per ogni altro compito, il **4** compreso; §7.7 riga 18 |
...
| 7 | ⛔ **negativa**: la spec non è toccata, `git diff --name-only <base>..HEAD -- docs/superpowers/specs/2026-08-06-sottoprogetto-1-kernel.md` non rende niente; nessuna riga ⚠️ o ⏳ della §8 passa a ✅; nessuna cifra nuova in prosa; `riferimenti.md` intatto. ⛔ **RICHIAMO DEL 2026-09-03: la PRIMA metà non vale più per il compito 3bis, e le altre tre restano INTERE.** Il proprietario ha **sospeso** il vincolo globale 1 per quel solo compito e per le sole **§8.3** e **§8.4** — voce **E10** dell'errata del [piano](../plans/2026-09-02-sottoprogetto-1-chiusura.md) — quindi quel comando rende ora la spec. ⚠️ **Per ogni altro compito, il 4 compreso, questa condizione si legge com'è scritta**: è il metro con cui la chiusura sarà giudicata, e un metro che invecchia in silenzio è il gotcha **#49**. §7.7 riga 18 |
...
compiti ordinati dalla §5.4, con ciò che ciascun commit può dire; la Definizione di
...
è un audit prima di essere una scrittura. ⚠️ **RICHIAMO DEL 2026-09-03: non è più in bozza.** Il
compito 4 ha scritto le righe 6 e 7 della §7.2, la §7.6 e la §7.9, e ha datato l'intestazione qui
sopra.
...
| Blocco della §1.3 | Il 2026-09-02, dal disegno | Rilanciato il 2026-09-03 | Diverge? |
|---|---|---|---|
...
⛔ **RICHIAMO DEL 2026-09-03: il capoverso qui sopra è il verbale del COMPITO 1, che non ha corretto niente — e il COMPITO 3bis ha poi corretto la §8.** Il proprietario ha **sospeso** il vincolo globale 1 per quel solo compito e per le sole **§8.3** e **§8.4** (voce **E10** dell'errata del piano); il catalogo **§7.4 non è stato toccato**, e **nessuna riga di catalogo è nata**. Ciò che è nato è un **caso** per una riga che esisteva dal 2026-08-10.

| # | Condizione | Esito | Evidenza letta o rilanciata il 2026-09-03 |
|---|---|---|---|
| 1 | ogni V in perimetro ha un controllo che gira in automatico | ✅ | le trenta righe del blocco **A** — 16 ✅ e 14 ⚠️ — lette una per una: **ventidue** hanno il controllo, **otto** no: `V5`, `V8`, `V9`, `V24`, `V25`, `V35`, `V36`, `V37`. I due conteggi si rifanno contando la colonna «Esito» della tabella qui sotto. ✅ **RICHIAMO DEL 2026-09-03 — le otto sono RISOLTE dal compito 3bis, e l'esito passa da ❌ a ✅. Le cifre qui sopra restano quelle dell'audit e non si riscrivono** (vincolo globale 9): `V5` col **caso `compile_fail`** che la sua riga di catalogo prometteva dal 2026-08-10 · `V8` con la **cella corretta**, che resta ✅ perché per un V l'autorità è il testo del vincolo (§8.1.3) · `V9`, `V25`, `V35` e `V37` **declassate a ⏳**, e il rimando in coda alla §0.7 dice che *«una riga rimandato non le viola»* · `V24` con la **metà verificata ristretta** ai permessi · `V36` **non toccata**, perché il suo ❌ nasceva dalla riga di catalogo `V5` senza caso e sparisce col caso. Decisione del proprietario, voce **9** di §7.8, voce **E10** dell'errata del piano |
| 2 | ogni controllo statico è stato visto fallire su una violazione deliberata, e poi tornare verde | ✅ | livello 1, blocco **H**: `40` casi, `40` `.stderr`, `0` vuoti, nessuna riga `SENZA STDERR`, e `cargo test --locked -p kernel --test compile_fail` verde. I quattro script: quattro rossi **letti**, e le revoche provate — `cmp` sulle tre che mutano un file, la cancellazione sulla quarta. La tabella qui sotto. ⚠️ **RICHIAMO DEL 2026-09-03: le due cifre di questa cella sono l'istantanea dell'AUDIT sul commit `9214255`, e non si riscrivono** (vincolo globale 9, voce **E12 ④** dell'errata del [piano](../plans/2026-09-02-sottoprogetto-1-chiusura.md)). Il soggetto e il commit sono resi espliciti qui perché la colonna porta un'altra data. Rilanciata lo stesso giorno **dopo** il compito 3bis: `ls crates/kernel/tests/compile_fail/*.rs \| wc -l` rende `41`, altrettanti `.stderr`, `0` vuoti — **uno in più**, ed è il caso che il 3bis ha scritto; `git ls-tree -r --name-only 9214255 crates/kernel/tests/compile_fail/` ne conta `40` e `40`, che è ciò che questa riga dice |
...
| 6 | `roadmap.md`, `tracciabilita.md`, lo stato degli spike e `HANDOFF.md` sono aggiornati **nello stesso passaggio** | ✅ | il passaggio unico del compito 4: **un** commit, e `git show --stat 1c6716c` nomina i **nove** file, che sono quelli della tabella della §5.2 e il piano di questa chiusura: `roadmap.md`, `README.md`, `HANDOFF.md`, `AVVIO-CHAT.md`, `COMPENDIO.md`, `tracciabilita.md`, `semi-dst.md`, questo disegno e il piano. ⚠️ **Lo stato degli spike è aggiornato nel senso che la §5.2 gli assegna: la tabella «Spike aperti» è VERIFICATA e invariata**, perché il sotto-progetto 1 non ne chiude nessuno (§1.3), e la §7.6 lo dichiara. ⛔ **Questa riga è scritta prima del commit e riletta dopo**, perché la sua evidenza è il commit stesso. ⚠️ **RICHIAMO DEL 2026-09-03:** qui stava `HEAD`, che si sposta al primo commit successivo e faceva marcire l'evidenza — il commit si **nomina** |
...
| `V24` | ⚠️ | test a esempi: il picco di VRAM **e** i permessi attivi si ricavano **rileggendo il giornale** | i permessi rileggono davvero: `is_granted` in `crates/kernel/src/permission.rs` chiama `journal.replay()`, provato da `nothing_is_granted_on_an_empty_journal` in `crates/kernel/tests/permission_triple.rs`. ⛔ Il picco no: `grep -rn 'VramPeak' crates/ --include=*.rs` lo trova in `crates/kernel/src/wire/worker.rs` e nei test del filo, e `a_vram_peak_survives_the_round_trip` è un giro di codifica e decodifica. Le chiamate a `journal.replay()` del kernel — `grep -rn 'journal.replay()' crates/kernel/src --include=*.rs` — stanno in `degradation.rs`, `permission.rs` e `reconcile.rs`, e nessuno legge il picco | ❌ sulla metà del picco — **stesso criterio di `V8`**: la cosa esiste ed è provata, ma non è quella che la cella nomina. ✅ **CHIUSA il 2026-09-03 dal compito 3bis:** la riga di §8.3 **resta ⚠️** e il **picco di VRAM esce dalla metà verificata** con un richiamo datato; i permessi restano la metà che regge |
| `V25` | ⚠️ | «il controllo gira a ogni commit e la **sonda scatta** — una chiamata di rete in `daemon` lo accende» | ⛔ nessuno script guarda `daemon`: `grep -n '^CRATES=' scripts/gate-deps.sh` rende `CRATES="kernel simulator"`, e `scripts/gate-attributes.sh` dichiara in un proprio commento che `platform`, `secrets` e `daemon` **non sono controllati**. `grep -n 'daemon' scripts/*.sh` non rende nessuna riga eseguibile | ❌ è la lacuna che declassò `V34` alla riga accanto, e su questa riga è aperta. ✅ **CHIUSA il 2026-09-03 dal compito 3bis:** la riga di §8.3 è **declassata a ⏳ rimandato**, innesco `B (3)` invariato. ⚠️ La lacuna del grafo di `daemon` resta aperta, ed è la voce **8** della §7.8 |
...
| `V35` | ⚠️ | test a esempi, «gli stessi che rendono ✅ `V37`» | ⛔ vedi la riga `V37`: il soggetto è lo stesso e non esiste | ❌. ✅ **CHIUSA il 2026-09-03 dal compito 3bis:** la riga di §8.3 è **declassata a ⏳ rimandato**, innesco `D (5)` invariato, insieme a `V37` da cui dipende |
| `V36` | ⚠️ | riga §7.4.1 C `V5` — un effetto senza classe non compila | ⛔ vedi la riga `V5`: la riga di catalogo su cui questa poggia non ha nessun caso che la eserciti | ❌. ✅ **CHIUSA il 2026-09-03 dal compito 3bis SENZA toccare la §8** (**E10**, **E13**): il ❌ nasceva dalla riga di catalogo `V5` senza caso, ed è sparito col caso — la cella di `V36` in §8.3 dice il vero da allora |
| `V37` | ✅ | test a esempi: la parte che §7.4.5 fa entrare comunque — il **tipo** del livello di confinamento, la sua dichiarazione per azione, la sua registrazione nel giornale | ⛔ il tipo non esiste: `grep -rniE 'confinement\|sandbox\|confin' crates/ --include=*.rs` rende **tre righe di commento** e nessun tipo, e una di quelle righe è `crates/platform/src/lib.rs`, che porta il comando e la propria risposta — `grep -rniE "pub trait (Clock\|Confinement)" crates/` **risponde zero**, rilanciato oggi. `Permission` in `crates/kernel/src/permission.rs` porta `tool`, `resource` e `operation`, e `PermissionDetail` in `crates/kernel/src/record.rs` porta `tool`, `resource` e `write`: nessun livello di confinamento | ❌ il soggetto nominato non esiste. ✅ **CHIUSA il 2026-09-03 dal compito 3bis:** la riga di §8.3 è **declassata da ✅ a ⏳ rimandato**, con l'innesco `D (5)` **nuovo**, che §8.6 pretende ora |
...
| `Q13` | **proprietà** | **gettone di conformità**, livello 1: la proprietà è resa non esprimibile | ❌ **divergenza dichiarata nella cella**, il 2026-08-08, «invece che nascosta»: il metodo assegnato è una verifica di proprietà su catene generate, e qui è sostituito da uno che la cella argomenta più forte. Resta una sostituzione, e §8.1.3 pretende che si dica. ✅ **CHIUSA il 2026-09-03 dal compito 3bis, e NON in §8:** la cura è la **§7.3**, che registra la condizione scritta più larga della decisione — sul precedente della condizione 4 nella stessa sezione. Il metodo qui non è **assente**: è **più forte**, ed è dichiarato dal 2026-08-08 (**E10**) |
...
| `Q23` | **statica + esempi** | la cella dichiara che **nessuna delle due** gira qui, «e per lo stesso motivo: nessuna porta esegue comandi»; ciò che entra sono i test a esempi di `V37` | ❌ **dichiarato nella cella stessa**: «ed è meno di quanto `design/08` chiede». ⚠️ e quei test a esempi non esistono: è la riga `V37` della condizione 1. ✅ **CHIUSA il 2026-09-03 dal compito 3bis:** la riga di §8.4 è **declassata a ⏳ rimandato**, innesco `D (5)` invariato, insieme a `V37` su cui poggiava |

### 7.3 Dove la condizione era scritta troppo larga

⚠️ **Le righe stanno nell'ordine in cui sono state scritte — la `4` dal compito 1, la `3` dal compito 3bis — e non in ordine numerico: riordinarle riscriverebbe un verbale** (vincolo globale 9; voce **E13** dell'errata del [piano](../plans/2026-09-02-sottoprogetto-1-chiusura.md)).
...
| **3** | la §0.7 dice: «ogni Q in perimetro è verificato col metodo che [design/08](../../design/08-strategia-di-test.md) gli assegna, **non con un altro**» | la cella di `Q13` in §8.4 **dichiara la sostituzione dal 2026-08-08** e la argomenta: il metodo assegnato è una **verifica di proprietà** su catene generate, e qui la proprietà è resa **non esprimibile** da un gettone di livello 1 — *«una proprietà provata su N catene lascia scoperta la N+1, un tipo no»*. È la stessa mossa di §5.3 punto 3, e §8.1.3 **pretende che si dica**: cosa che quella cella fa | la condizione è soddisfatta **in quella lettura, e in quella soltanto**: presa alla lettera rifiuterebbe una sostituzione **più forte**, dichiarata, e scritta nella cella **dal 2026-08-08** — declassare una riga il cui controllo **eccede** il richiesto direbbe il falso nella direzione opposta. ⛔ **La §0.7 non si tocca**: è spec, ed è del proprietario — vincolo globale 1 del piano. ✅ **Deciso dal proprietario il 2026-09-03**, voce **9** di §7.8; il numerale del testo dettato è **tolto** e sostituito dalla data, voce **E13** dell'errata |
...
| la §6 ha perso **esattamente** quelle righe | `diff` fra la §6 di prima, presa da una copia fatta **prima** della scrittura, e quella di dopo | tolte le `378` righe, aggiunto il solo puntatore. ⚠️ `git show --numstat 3f32d5e` ne conta `377`: la 378ª è la riga **vuota** di testa, che il diff allinea con quella rimasta sotto l'intestazione |
...
`git show --stat 1c6716c`, che nomina i nove file. ⚠️ **RICHIAMO DEL 2026-09-03:** qui stava
`HEAD`, che si sposta al primo commit successivo — un verbale nomina il **commit**.
...
| il [piano](../plans/2026-09-02-sottoprogetto-1-chiusura.md) | la tabella della posizione, e l'errata con **E18** ed **E19** | **nulla tolto, e nessun richiamo**: la cella della posizione passa da ⬜ a ✅ e l'errata guadagna due righe — nessuna riga vecchia diventa falsa. Rilanciato il 2026-09-03: `git diff --numstat 1c6716c^ 1c6716c -- docs/superpowers/plans/2026-09-02-sottoprogetto-1-chiusura.md` rende `3 1`, cioè la sola riga della posizione riscritta |
...
| 16 | §4.3: il blocco *«nominava 58 codici `E<n>` distinti e ognuno ha una riga nell'errata»* | **ha retto sul merito, e il comando del piano ne conta uno in più.** `grep -oE 'E[0-9]+' \| sort -u \| wc -l` rende `59` il 2026-09-03 sul commit di base `20bb9bf`, e `59` anche su `f61fd53`; il 59° è `E0004`, che sta dentro `` error[E0004] `` ed è un codice del **compilatore**, non una voce d'errata. Le altre `58` hanno ciascuna la propria riga, verificate una per una. La prosa del disegno era giusta; il `grep` del Passo 2 non distingue le due popolazioni |
| 17 | §4.7 e il Passo 4: *«il blocco viene da un file CRLF, quindi si convertono i CR con `tr -d '\r'` prima»* | ⚠️ **arriva già senza `CR`, e la conversione è un `no-op` — misurato, non supposto.** Il `sed` di questa Git Bash legge in modo testuale e **toglie** i `CR`: `sed -n '645,646p' docs/COMPENDIO.md \| od -c` non ne mostra nessuno dove `head -n 646 \| tail -n 2 \| od -c` ne mostra due. ⛔ **La conseguenza non è cosmetica:** il blocco **D** conta perciò byte **LF**, `39257`, mentre i byte che la sua campata pesa davvero nel file sono `39642` — uno in più per riga. ⚠️ **E la campata non è ciò che esce:** comprende l'intestazione e il capoverso del marcatore, che **restano** in §6 per **D1** ed **E5**. Quanto è uscito, e quanto il compendio ha perso netto, li dicono le prime due righe della §7.5 coi loro comandi. La cautela del piano è stata applicata lo stesso e la misura `tr -cd '\r' \| wc -c` → `0` la conferma; il comando del blocco **D** resta dipendente dall'ambiente, ed è la voce **12** della §7.8 |
| 18 | §1.2 e §6.3 condizione **7**: la chiusura *«non tocca la spec»*, e il comando negativo *«non rende niente»* | ⛔ **smentito, e dal proprietario invece che dall'esecuzione.** La voce **9** di §7.8 è stata decisa il 2026-09-03 con la **via A**, e il vincolo globale 1 **sospeso** per il solo compito 3bis e per le sole **§8.3** e **§8.4** (voce **E10** dell'errata del [piano](../plans/2026-09-02-sottoprogetto-1-chiusura.md)): `git diff --name-only 9214255..HEAD -- docs/superpowers/specs/2026-08-06-sottoprogetto-1-kernel.md` rende ora quel file. ⚠️ **Le due sezioni approvate NON sono state riscritte** — vincolo globale 9: ciascuna porta accanto il proprio **richiamo datato**, perché una Definizione di «fatto» è il **metro** con cui il compito 4 sarà giudicato, e nessuno rilegge il metro. È il gotcha **#49**, colto dalla revisione 1 del compito 3bis (rilievo **I2**) e non dall'esecuzione che l'aveva prodotto |
| 19 | §5.2, riga di [`COMPENDIO.md`](../../COMPENDIO.md): per l'intestazione, *«col SOTTO-PROGETTO 1 CHIUSO», la data* | ⛔ **l'esecuzione ha aggiunto DUE PAROLE, e le ha scritte prima che fossero vere.** L'intestazione dice *«dal piano della chiusura eseguito **e rivisto**»*, e l'ha scritta il **compito 4 stesso**, nel commit `1c6716c`, quando nessuna revisione del compito 4 esisteva ancora — quella dell'intero ramo è stata dispacciata **dopo**. ✅ **La frase è vera adesso**, e non si riscrive: il compito 4 ha avuto le sue revisioni finali, e l'ondata di correzione del 2026-09-03 con la propria ri-revisione le completa. Renderla **più debole del vero** sarebbe un difetto quanto addolcirla — è la ragione che la §7.9 porta in testa, *«chiudere troppo è un modo di fallire quanto chiudere troppo poco»*. ⛔ **Ciò che si registra qui è il difetto TEMPORALE:** per il tempo fra `1c6716c` e quell'ondata l'unica lettura obbligatoria ha dichiarato **fatto** ciò che non era ancora fatto. Trovato dalla revisione finale del ramo, rilievo **C1** |
| 20 | §5.2, riga di [`AVVIO-CHAT.md`](../../AVVIO-CHAT.md): **una riga sola** — *«e il sotto-progetto 1 è in corso»*, per sottrazione | ⛔ **il disegno era SOTTO-SPECIFICATO, e l'esecuzione lo ha seguito alla lettera.** Quel messaggio prometteva anche *«si lavora a un TRAGUARDO NUOVO»*, e chiamava `brainstorming` *«la PRIMA del traguardo nuovo»*: la chiusura rende false anche quelle, perché dopo il sotto-progetto 1 non c'è un traguardo ma i **due brainstorming** che la §6 del compendio nomina. ✅ **Corretto e non riproposto al proprietario:** la **direzione** era già sua e già scritta nella §6, mancava l'esecuzione — precedente dell'audit del 2026-08-27, *«un'esecuzione lasciata a metà da un disegno approvato si completa, non si ripropone al proprietario»*. Le **due** affermazioni vive sono **tolte per sottrazione**, con **un** richiamo datato che le copre entrambe; le occorrenze rimaste stanno dentro **richiami datati** e non si riscrivono. Comando: `grep -ni 'traguardo nuovo' docs/AVVIO-CHAT.md`, rilanciato il 2026-09-03. Trovato dalla revisione finale del ramo, rilievo **I4** |
...
⚠️ **RICHIAMO DEL 2026-09-03, dalla ri-revisione finale:** *«il compito 4 non ne apre
nessuna»* vale per il compito **eseguito**; le voci **16**, **17** e **18** le hanno aperte le
**due revisioni finali** del ramo e l'ondata che le ha chiuse. La riga sopra **non si riscrive**:
è un verbale, e questo è il suo richiamo.
...
| 6 | **le otto righe della §8.3 il cui controllo non esiste in `crates/`** — `V5`, `V8`, `V9`, `V24`, `V25`, `V35`, `V36`, `V37`. È la classe del finding AUD-026, quello che declassò `V34`; nessuna è stata corretta qui. ⚠️ **RICHIAMO DEL 2026-09-03:** *«nessuna è stata corretta qui»* era vero quando il **compito 1** scrisse questa voce, ed è **superato dalla voce 9** di questa stessa tabella — il proprietario ha deciso la via **A**, il compito **3bis** l'ha eseguita, e la §7.2 marca le otto righe *«CHIUSA il 2026-09-03»*. La voce **non si riscrive**: è un verbale | §7.2, tabella della condizione 1, riga per riga col comando | il proprietario: la §8 e il catalogo §7.4 sono spec. ⚠️ **Il rimedio non è unico** — riscrivere la cella, scrivere il controllo che manca, o ri-giudicare lo stato della riga — e sceglierlo è suo |
| 7 | **le due righe della §8.4 verificate con una tecnica diversa da quella che `design/08` assegna** — `Q13` e `Q23`, entrambe **dichiarate nella cella stessa** e non nascoste. ⚠️ **RICHIAMO DEL 2026-09-03:** vale ormai per la sola `Q13`. `Q23` è stata **declassata a ⏳ rimandato** dal compito 3bis, quindi non è più verificata con **nessuna** tecnica, e il rimedio che questa voce propone si applica a metà del proprio soggetto — `grep -n '^. Q23 ' docs/superpowers/specs/2026-08-06-sottoprogetto-1-kernel.md`, rilanciato quel giorno, rende `⏳ rimandato`. La voce **non si riscrive**: è un verbale | §7.2, tabella della condizione 3 | il proprietario: o `design/08` cambia il metodo assegnato, o la §8.4 dichiara la divergenza come permanente. Nessuno dei due è un rimedio di questa chiusura |
...
| 9 | ⛔ **le condizioni 1 e 3 della §0.7 sono ❌ il 2026-09-03**, e la §0.7 dice che il sotto-progetto è chiuso quando **tutte** le sue condizioni sono vere | §7.2, le due righe con la propria evidenza | ⛔ **il proprietario, e prima del compito 4**: il compito 4 è l'unico che può scrivere «chiuso», e questo verbale non decide al suo posto. ✅ **CHIUSA il 2026-09-03: il proprietario ha deciso la VIA A** — la §8 smette di promettere controlli che non esistono, e il compito 4 va poi com'è scritto. ⛔ **Le tre eccezioni misurate NON sono ricopiate qui:** la loro casa unica è la voce **E10** dell'errata del [piano](../plans/2026-09-02-sottoprogetto-1-chiusura.md). Eseguita dal **compito 3bis**, che il proprietario ha aggiunto al piano lo stesso giorno; le due righe delle condizioni 1 e 3 della §7.2 portano ora ✅ col proprio richiamo datato |
...
| 12 | ⛔ **il comando del blocco D della §1.3 misura il proprio testo e non il file, e la differenza dipende dall'ambiente**: il `sed` di questa Git Bash toglie i `CR`, quindi la campata che il blocco delimita rende `39257` byte LF dove quella stessa campata ne pesa `39642` in CRLF. Su una macchina il cui `sed` conservasse i `CR` lo stesso comando renderebbe un altro numero, e nessuna delle due letture è sbagliata: sono due domande diverse. ⚠️ **Né l'una né l'altra è quanto esce:** la campata comprende le righe che restano, e la §7.5 le separa | §7.5, riga «byte del blocco», e §7.7 riga 17 | il proprietario: se il blocco **D** debba misurare **il file** — per esempio con `python -c` o con `head`/`tail` invece del `sed` — o restare com'è, dichiarando che conta il **testo**. Non è un difetto oggi: il blocco è già archiviato, e la §7.5 separa la campata da ciò che ne è uscito |
...
| 16 | ⚠️ **il commit `938eb37` ha RISCRITTO due verbali, oltre a dar loro il richiamo datato:** la riga 6 della §7.2 e il capoverso della §7.6 dicevano `git show --stat HEAD`, e dicono ora `1c6716c`. La lettera stretta del vincolo globale 9 avrebbe tenuto il verbale parola per parola, mettendo la correzione **solo** nel richiamo. ⚠️ **La scelta è difendibile** — `HEAD` è un puntatore che **marcisce da solo**, e la dottrina di [`CLAUDE.md`](../../../CLAUDE.md) è che un puntatore così *si toglie, non si ricorregge* — ma resta la riscrittura di un verbale, ed è la specie su cui questo ramo ha già dovuto decidere due volte | §7.2 riga 6 e §7.6, coi rispettivi richiami del 2026-09-03 | il proprietario: se un verbale la cui **evidenza marcisce da sola** vada **riscritto** o solo **affiancato** dal richiamo. ⛔ **Registrata e non presa:** l'ondata di correzione del 2026-09-03 non ha né revocato né esteso quella riscrittura |
| 17 | ⚠️ **la cella di `V8` poggia sul «TESTO del vincolo … in tutte le sue parti» e non nomina dove viva il *default conservativo*.** Il vincolo è *«ogni run ha un tetto, con default conservativo»*: la cella riscritta il 2026-09-03 nomina il **tetto** — consegnato e provato — e non il **valore**, che è la costante `EXECUTOR_TURN_LIMIT` di `crates/daemon/src/main.rs` e vive quindi **fuori** dalle due crate che il cancello misura. Rilanciati il 2026-09-03: `grep -rn 'EXECUTOR_TURN_LIMIT' crates/ --include=*.rs` rende **solo** righe di `daemon`, e `grep -n '^CRATES=' scripts/gate-deps.sh` rende `CRATES="kernel simulator"`. ⚠️ **Non è un ❌:** `Parameters` non ha `Default` per **decisione** — il caso `parameters_have_no_default.rs` — la conservatività è argomentata nel sorgente, e il sorgente **dichiara da sé il residuo** (`DECLARED RESIDUAL -- IT DOES NOT COVER THE VALUE OF EXECUTOR_TURN_LIMIT`, `crates/daemon/src/main.rs`). ⛔ **La §8 non è toccata** — vincolo globale 1 — quindi il fatto sta qui e non lì | §8.3, riga `V8`, contro `crates/daemon/src/main.rs` | il proprietario: se la cella di `V8` debba nominare il default, o se *«in tutte le sue parti»* della §8.1.3 vada letto sul solo perimetro che il cancello misura. È la stessa famiglia della voce **8** |
| 18 | ⚠️ **una cifra nuova in prosa che il vincolo globale 4 non voleva, e che di qui NON si può togliere:** il compito 3bis ha scritto in [`porta-di-qualita.md`](../../porta-di-qualita.md), nella nona ricontata del blocco **C** di §7.4.1 (commit `3042a19`), *«DICIANNOVE righe su diciannove»*. ✅ **Il danno è nullo**: la cifra sta dentro un **richiamo datato**, segue il registro delle ricontate che quella pagina tiene, e nomina il caso che la rende vera. ⛔ **Ma la clausola *«nessuna cifra nuova in prosa»* della §6.3 condizione 7 è dichiarata INTERA e non lo è**, e quel file è fuori dal perimetro del **compito 4** e di questa ondata — il **3bis** vi scrisse per l'eccezione che `E10` dichiara — — vincolo globale 3, la stessa ragione che la §7.6.3 dichiara. ✅ **La gemella è stata TOLTA:** *«le campagne sono CINQUE»* di [`semi-dst.md`](../../semi-dst.md), il 2026-09-03, perché quel file è in perimetro | `docs/porta-di-qualita.md`, blocco **C** di §7.4.1 | il proprietario: quella pagina la tocca lui, salvo le righe false che un audit apre |
...
| | |
|---|---|
| le voci della **§7.8** restano aperte, **salvo la 9** | la colonna «chi la chiude» nomina il proprietario, un sotto-progetto futuro, o una misura che ancora non esiste. L'unica chiusa è la **9**, e a chiuderla è stata una decisione del **proprietario** — la via **A** — eseguita dal compito 3bis, non questa chiusura. La chiusura del sotto-progetto 1 è contro la **§0.7**, non contro l'insieme delle voci aperte. ⚠️ **RICHIAMO DEL 2026-09-03:** qui stava *«restano tutte aperte»* e *«nessuna è chiusa qui»*, mentre la voce 9 porta ✅ **CHIUSA** della stessa data: era una **sottovalutazione**, che qui costa quanto addolcire. Corretto per sottrazione |
