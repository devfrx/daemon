...
il piano, e l'attività non ha il problema perché vive nel kernel (pezzo 6 della §3 della stella polare). ✅ **RICHIAMO DEL 2026-09-20, compito 12 del piano della parte 2 (D41, P-68):** la costruzione dell'arbitro **non si sposta** — nessuna casa condivisa regge: il kernel non nomina un default (ADR-0034), le crate restano cinque, una quota VRAM non è dell'OS, e un `lib.rs` nel daemon porterebbe il cablaggio di produzione in un attrezzo — quindi il finto la **riscrive** coi propri letterali, e le tre case (`crates/daemon/src/main.rs`, la campagna del compito 10, `gui/fake-core/src/main.rs`) si nominano a vicenda; decisione del proprietario del 2026-09-14. ⛔ **RICHIAMO DEL 2026-09-21, rilievo E118 della revisione del compito 12 — «le tre case … si nominano a vicenda» è falso in ENTRAMBE le metà.** Le case della **forma** sono **due**, e a dirlo è il comando e non questa riga: `grep -rn 'const AUDIO_RESERVATION' crates/ gui/ --include='*.rs' | grep -vE '^[^:]+:[0-9]+: *//[!/]'` rende il daemon e il finto. La campagna del compito 10 costruisce le proprie concessioni con un aiutante **generico** `standing_grant` e la forma non ce l'ha affatto. E «si nominano a vicenda» era vero di **una** casa sola: il commento del daemon non nomina nessuno. ✅ **Il merito di D41 NON si riapre** — la costruzione dell'arbitro resta riscritta, per le ragioni qui sopra, che la misura non tocca: cambia il **censimento**, non la decisione. La cura, applicata nel sorgente lo stesso giorno, è quella di `CLAUDE.md`: l'elenco si toglie e resta il comando.
**Assunto:** niente.
