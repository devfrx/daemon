...
✅ **RICHIAMO DEL 2026-09-10, alla chiusura della parte 1 del piano:** SP-8 eseguito — M1–M5 e Q1–Q4 in ADR-0029, chiuso **Electron**; le otto mosse in `spikes/RISULTATI.md`, `dockview` resta; la parte 2 del piano si scrive ora, coi numeri. Due fatti del frontend minimo per chi scrive la SPA: `dockview-core` 8.2.0 **non spedisce il foglio di stile** — il pacchetto `dockview` 8.2.0 entra per il solo `dist/styles/dockview.css`, l'API resta `dockview-core` e il ponte resta nostro (E2 del piano) — e l'emettitore di `spikes/gui-ipc/` è riusato **com'è**, chiavi JSON italiane comprese: il tipo `Wire` dello spike le nomina perché sono le chiavi del produttore, e muore con lo spike; la SPA parla `bincode` sull'`ipc` del kernel, non questo JSON (decisione 10 della quinta chiusura del piano).
