...
| la suite di conformità | `crates/kernel/tests/ipc_contract.rs`, inclusa da `crates/platform/tests/ipc_contract_real.rs` con `include!`, come `reactor_contract` e `journal_contract`; gira su `FakeGui`, `DyingGui` e il trasporto vero, questo con un pari vero su un thread ✅ **RICHIAMO DEL 2026-09-17, compito 2 (D82):** vive in `crates/kernel/tests/contract/ipc.rs` (non un bersaglio di `kernel`), gira sul **solo trasporto vero** e senza bugiardi — ogni sua promessa vuole un pari che scrive byte | è lei la prova; i bugiardi provano che morde |
