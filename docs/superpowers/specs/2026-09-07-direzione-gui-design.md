...
dice la §6 del [compendio](../../COMPENDIO.md), in un posto solo. ✅ **La rilettura del proprietario è FATTA il 2026-09-09** —
in chat, voce per voce, con le sei voci sue decise, tutte A (decisioni 41–47 qui sotto); il prossimo passo lo dice la §6 del compendio.
...
| 40 | **alla chiusura della sedicesima ripresa, 2026-09-09** — i due disegni scritti sul posto: la rilettura del proprietario in una sessione nuova (A), o subito, in quella chat, voce per voce (B) | **A.** La sessione nuova apre con la domanda minima della §10 del disegno del 2 — *«i due disegni sono riletti?»* — e le voci per il proprietario stanno lì; nessuna domanda resta pendente |
| 41 | **alla rilettura dei disegni, 2026-09-09** — la voce 5 della §9 del 2, la prontezza I/O del reattore, presentata in forma piena (esiste, arriva, regge, i cinque criteri, verificato/dedotto/assunto): A, nel 2 il daemon guarda il filo a ogni giro e dorme un tick consegnato, e il piano misura il processore a riposo **senza soglia**; B, il reattore impara la prontezza I/O già nel 2 — contratto cambiato, undici implementazioni, la suite di conformità e il simulatore | **A.** Un cliente oggi; il numero arriva dal piano; il reattore cambia una volta sola quando i clienti saranno due, col 3 e la rete. Letto nel codice quel giorno: tre operazioni in `crates/kernel/src/ports/reactor.rs`, undici `impl Reactor for` |
| 42 | **alla rilettura dei disegni, 2026-09-09** — la voce 6 della §9 del 2, dove vive il contatore dei numeri progressivi (`ClientId` oggi, `StepId` col 3): A, un tipo nuovo del kernel, seminato dal daemon con `replay` sopra l'ultimo numero del giornale e consegnato al trasporto (ADR-0034), com'è già nella §3; B, un'operazione `allocate` nella porta `journal` — la tabella normativa §4.1, due implementazioni e la suite coi bugiardi | **A.** Un cliente oggi; la regola «un contatore solo» è rispettata; A si rovescia in B spostando il tipo dietro un'operazione, senza toccare il trasporto. Letto nel codice quel giorno: sei operazioni della porta, nessuna che allochi; i commenti di `ports/journal.rs` (righe 68–78) e `ports/ipc.rs` (121–143); nessun numero creato dal daemon. Dedotto e detto: un numero di client ripetuto dopo un riavvio è innocuo, la GUI vive quanto il processo (ADR-0004) |
| 43 | **alla rilettura dei disegni, 2026-09-09** — la voce 11 della §9 del 2, AUD-004, l'ADR del proprietario che decide se le difese di ADR-0015 (testo integrale all'approvazione, impronta, sospensione se cambia) valgano anche per le skill dichiarative: A, resta **in parallelo** al 2 e **prima** del brainstorming del 13, com'è deciso dal 2026-09-04 nel disegno della knowledge base; B, si scrive prima del piano del 2 | **A.** Il 2 non tocca skill — nei due disegni la parola compare, per il prodotto, solo nella riga 16 della Chat, del 13 e del 3 — e il suo registro è quello delle **funzioni**, non delle guide; AUD-004 sbarra il 13, non il 2; riaprire il «quando» senza un fatto nuovo sarebbe contro la regola di `CLAUDE.md`. Letto quel giorno: la riga dei 73 (chiuso con `5d66088`, la decisione registrata e non presa), la scheda, le righe del disegno della knowledge base |
| 44 | **alla rilettura dei disegni, 2026-09-09** — la voce 13 della §9 del 2, prima metà: X-1 dell'audit, la CI che gira solo su `ubuntu-latest` mentre ADR-0002 dice «sviluppiamo e testiamo su Windows», e col 2 il trasporto del filo in `platform` è codice con due metà: A, una matrice nel workflow, Linux e Windows, lo stesso cancello; B, resta solo Linux e la metà Windows la prova il cancello sulla macchina del proprietario | **A.** Poche righe di YAML, minuti gratis (repo pubblico, letto dall'API di GitHub quel giorno), il cancello gira già in Git Bash su Windows (`GATE GREEN` sulla macchina del proprietario lo stesso giorno). L'esecuzione è un **compito del piano del 2**, accanto al passo `setup-node` della §8. Da provare al piano, nelle due direzioni: che il runner Windows onori `rust-toolchain.toml` e che Git Bash vi lanci `gate.sh`. ✅ **RICHIAMO DEL 2026-09-22, dal compito 16 del piano della parte 2 (D88): ESEGUITA** — `strategy.matrix.os` con `ubuntu-latest` e `windows-latest`, `fail-fast: false`, e `shell: bash` sull'ultimo passo (E207); che il runner Windows onori `rust-toolchain.toml` e che Git Bash lanci `gate.sh` lo dice la corsa, e il criterio del 16 manda a guardarla (P-111). X-1 chiusa nella sua riga dell'audit |
| 45 | **alla rilettura dei disegni, 2026-09-09** — la voce 13 della §9 del 2, seconda metà: X-3 dell'audit, nessuna scansione degli avvisi di sicurezza sulle dipendenze (`gate-deps.sh` misura *quali* crate, mai *come stanno*; l'avviso di `bincode` restò invisibile sette mesi), e col 2 il secondo mondo npm: A, un passo per mondo dentro il cancello — `cargo audit` in `gate.sh`, `npm audit` in `gate-gui.sh` — così cancello locale e CI dicono la stessa cosa; B, resta aperta | **A.** È la cura alla radice che la riga dello stack del compendio nomina dal 2026-08-31; niente `cargo deny`, perché la lista di ADR-0031 la fa già `gate-deps.sh`. Letto alle fonti quel giorno: `cargo-audit` 0.22.2 (2026-06-05) e `cargo-deny` 0.20.2 (2026-07-09) su crates.io; nel file di configurazione d'esempio di `cargo audit` un avviso «non mantenuto» è un **warning** per default e diventa rosso solo con `deny = ["unmaintained"]`, quindi `bincode` resta com'è deciso; `npm audit` esce con errore su ogni vulnerabilità e `--audit-level` fissa la gravità che fa rosso. Costi dichiarati: il passo vuole la **rete**, e può fare rosso senza un commit perché il mondo ha pubblicato un avviso. Compito del piano del 2, versioni rilette quel giorno; la durata e il rumore di `npm audit` sulle dipendenze di sviluppo si misurano lì. ✅ **RICHIAMO DEL 2026-09-22, dal compito 16 del piano della parte 2 (D88): ESEGUITA** — `cargo audit` in `gate.sh` accanto a `gate-deps.sh` (D68, D69), `cargo audit --file fake-core/Cargo.lock` e `npm audit` senza `--audit-level` in coda a `gate-gui.sh` (D83, D71); misurato il 2026-09-22: `1 allowed warning found` per `bincode` e uscita 0 sui due lockfile, `found 0 vulnerabilities` per npm. X-3 chiusa nella sua riga dell'audit |
| 46 | **alla rilettura dei disegni, 2026-09-09** — la decisione 9 del coordinatore nella §10 del 2, i sette nomi inglesi fissati nella tabella della §1 del 2: la settima porta `custody` col tratto `Custody`, le operazioni `keep` e `retrieve`, la chiave `CustodyKey::Layout`; il modulo `registry`; i messaggi della §4 da provvisori a fissati; la lista dei passi `Steps` — A, restano; B, il proprietario ne cambia uno o più | **A.** Letto nel codice quel giorno: le sei famiglie di `ports/mod.rs` e i loro verbi corti (`accept`, `send`, `receive`, `intent`, `replay`, `prune`…), i moduli piatti del kernel, le due varianti `Request` e `Verdict`; nessuna collisione per `custody`, `registry` e `Steps` (`grep -rn` su `crates/`, solo dentro commenti). Dedotto e detto: `Layout` due volte — `IpcMessage::Layout` e `CustodyKey::Layout` — si legge bene, due enum che dicono la stessa cosa. Dopo il primo codice, un nome è del piano |
| 47 | **alla rilettura dei disegni, 2026-09-09** — la decisione 2 del coordinatore nella §10 del 2, la §1 del 2 scritta **per rimando** alla §3 di questo file, senza ricopiare i nove pezzi né il «non costruisce»: A, resta per rimando; B, la §1 ricopia le due liste | **A.** La regola di `CLAUDE.md` — un puntatore che vive in più documenti si toglie — e la §6 di questo file; letto quel giorno: la tabella dei nove pezzi vive in un posto solo (`grep -c` 1 e 0), la §1 del 2 vi punta con quattro righe. Costo dichiarato: chi legge il perimetro del 2 apre due file. Con questa la **rilettura è chiusa**: sei voci, tutte A, decisioni 41–47, ogni risposta scritta nei due disegni e, per X-1 e X-3, nell'audit, un commit l'una |
| 48 | **alla chiusura della rilettura, 2026-09-09** — dove si scrive la parte 1 del piano del 2 (punti 5–7 della §10 del 2): A, in una sessione nuova, perché questa porta già la lettura d'apertura intera più sette giri e il piano vuole un'altra lettura grande e il pre-controllo che ha trovato un difetto in ogni compito finora; B, subito, nella stessa sessione | **A.** Una fase per sessione, la regola del proprietario; lo stato è tutto nei file tracciati. La sessione nuova apre con la lettura obbligatoria e i due disegni per intero, poi i punti 5–7 della §10 del 2; nessuna domanda resta pendente |
...
6), e che il client rifiutato resti nella tabella fino al `Disconnected` (⛔ **RICHIAMO DEL 2026-09-18, compito 7 del piano della parte 2 (R3-12):** il client rifiutato **non** resta nella tabella: il core lo dimentica nello stesso giro in cui gli manda `StaleBuild`, il suo `Disconnected` non arriva a nessuno, e non serve — non tiene nulla); **assunto:** niente; stessa forma dei diagrammi di
...
disegno per messaggio, il minimo che dice ciò che le tabelle della §4 e della §5 del 2 non mostrano — l'ordine, e chi apre il passo. ✅ **confermato dal compito 7 del piano della parte 2, 2026-09-22:** l'accoglienza e il riinvio dopo ogni invocazione sono asseriti dalle sonde di `kernel::serving`. ⚠️ **Con una precisazione:** il client rifiutato resta in `clients` con un altro `Stage`, non in una tabella a sé, e `attending` non lo comprende — il doc di `Core::attending`
...
| 2 | la policy VRAM attiva e il budget allocato sul totale; il totale è il budget allocabile: tutto meno la quota audio e la quota di presentazione ⛔ **RICHIAMO DEL 2026-09-18, compito 7 del piano della parte 2 (D20):** `Policy` porta **`allocated`** — quanto i libri dell'arbitro impegnano, **le due quote permanenti comprese** — e **`total`**, la macchina consegnata. Il denominatore *«tutto meno la quota audio e la quota di presentazione»*, che l'ultima colonna di questa riga marca già **dedotto**, **non si costruisce**: il kernel non ha i due addendi e il doc di `Parameters::total_vram` argomenta di non consegnarli; e sottrarle nasconderebbe due concessioni **con un titolare** (ADR-0033), che è *«la sottrazione non è un'esenzione»*, gotcha **#4**, commesso allo strato che l'utente guarda. Se il proprietario vorrà il denominatore netto, i due addendi diventano parametri consegnati. | ADR-0006 · ADR-0005 · ADR-0033 · `Policy` · righe «Budget VRAM esplicito» e «Policy differenziata remoto vs locale» | 2 | verificato; le due quote sottratte visibili: dedotto |
...
prima della SPA. ✅ **confermato dal compito 5, 7 e 13 del piano della parte 2, 2026-09-22:** una tabella e una chiave (5, promessa 1); il core risponde a ogni `SaveLayout` con ciò che tiene (7, `keep` → `Layout`); il pacchetto è strutturato solo nella SPA, i default restano in `gui/`, il salvataggio è automatico e lo store confronta i byte che tornano con quelli mandati (13: `VIEWS`, `settle`, D80, D89)
...
niente di nuovo — `toJSON` è già nella §2. ✅ **confermato dal compito 1–16 del piano della parte 2, 2026-09-22:** l'ordine dei pezzi è quello della tabella della posizione del piano; la SPA è nata contro il ponte finto (11–13) e il core finto (12) prima del daemon in produzione (9); i passi all'accoglienza e dopo ogni invocazione: compito 7
...
| 47 | **§8** — niente cache npm in CI | una riga che si aggiunge quando la CI misura che serve: due meccanismi per una misura sola si comprano quando la misura c'è. Costo: `npm ci` scarica a ogni corsa ✅ **RICHIAMO DEL 2026-09-22, compito 17 del piano della parte 2 (D88, E213) — dovuto dal **15**:** la CI scrive `package-manager-cache: false` per esteso (D66): la cache resta spenta per dichiarazione, non per un campo assente |
...
| 51 | **§9, proposta alla chiusura della quattordicesima ripresa** — il renderer di markdown della chat: `markdown-it` (15.0.1, 2026-08-27, MIT), non `marked` né `micromark` | letto **dentro il pacchetto** il 2026-09-09: il preset `default` ha `html: false` e `linkify: false`, e `validateLink` rifiuta `vbscript:`, `javascript:`, `file:` e `data:` (tranne le immagini); il preset `commonmark` ha `html: true` e **non si usa**. `marked` (18.0.12) è più scaricato ma senza sanitizzazione propria: vorrebbe `dompurify`, una dipendenza in più per la sicurezza; `micromark` (4.0.2, 2025-02) è il motore di una pila più grande. Costo: se un modulo vorrà l'HTML del modello, è una decisione nuova, non un'opzione ✅ **RICHIAMO DEL 2026-09-22, compito 17 del piano della parte 2 (D88, E213) — dovuto dal **14**:** `markdown-it` è la dipendenza della Chat in `gui/package.json`; la versione appuntata la dice `grep '"markdown-it"' gui/package.json` (vincolo globale 8) |
| 52 | **§9** — `vitest` 4.1.11 (2026-08-18, tag `V4`) e non la 5.0.0 del 2026-09-03 | «novità non è maturità»: sei giorni; la 5 si prende quando il piano la rimisura. Costo: un aggiornamento di major al piano o dopo ✅ **RICHIAMO DEL 2026-09-22, compito 17 del piano della parte 2 (D88, E213) — dovuto dall'**11**:** `vitest` 4.1.11 è appuntato in `gui/package.json`, e `npm test` (`vitest run`) gira in `gate-gui.sh` |
...
`anthropic-skills:dev-communication`, `superpowers:writing-plans`; `anthropic-skills:design-docs` governa i diagrammi. ✅ **RICHIAMO DEL 2026-09-22, compito 17 del piano della parte 2:** il piano è **scritto** (2026-09-15), **rivisto** e **eseguito** — il passo dopo lo dice la §6 del compendio, in un posto solo
