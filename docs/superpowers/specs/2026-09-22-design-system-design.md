...
coppia, e non una lista tenuta a mano delle coppie che i componenti usano. ⚠️ **RICHIAMO DEL 2026-09-23, dalla revisione del
piano (R1-8), scelta A del proprietario:** alla lettera la frase fallisce per costruzione — nello scuro `--color-text-on-ok`
e `--color-bg` sono lo stesso gradino, `--ref-neutral-5` — e il test accoppia per **famiglie**, dai nomi e sempre senza una
lista a mano: sono le 176 coppie approvate con la tavola, e la regola la scrive il compito 1 del piano (P-1). Quanti ruoli porta ciascun tema lo dice il comando,
sui due temi:
...
| Regola | Il controllo |
|---|---|
| in `components/` **nessun import** di `pinia`, `stores/`, `panels/`, `frame/`, `transport/`. ⚠️ **Richiamo del 2026-09-23 (R2-11, scelta A del proprietario):** le regole sono **due**, perché `Confirm.vue`, che questa sezione tiene in `components/` come pezzo composto, legge due negozi: i pezzi di base — `Base*.vue` e `icons.ts` — non importano `pinia`, `stores/` né gli strati sopra; e tutto `components/`, tranne i `*.test.ts`, non importa `panels/`, `frame/`, `transport/` | `no-restricted-imports` di ESLint, limitato a `components/` |
...
| | |
|---|---|
| **il tema** | `themeAbyss` **esce** da `frame/dock.ts`, dove oggi sta sulla riga `theme: themeAbyss`; entra un `DockviewTheme` **nostro**: `className: "dockview-theme-harness"`, `colorScheme` che segue `data-theme`, `gap` letto dal token `--space-3`. Il tipo, in `dockview-core` 8.3.1, porta `name`, `className`, `colorScheme`, `gap`, `edgeGroupCollapsedSize`, `dndOverlayMounting`, `dndPanelOverlay` — letto in `gui/node_modules/dockview-core/dist/cjs/dockview/theme.d.ts` il 2026-09-23. ⚠️ **Richiamo del 2026-09-23 (R3-22):** quei sette non sono tutti — ci sono anche `dndTabIndicator`, `dndOverlayBorder`, il campo che il tema usa per `--dv-drag-over-border`, cioè il bordo della zona d'arrivo, `tabGroupIndicator` e `tabAnimation`; quanti sono lo dice `awk '/^export interface DockviewTheme \{/,/^\}/' gui/node_modules/dockview-core/dist/cjs/dockview/theme.d.ts \| grep -cE '^\s+[a-zA-Z]+\??:'` |
...
| **dove c'è** | su questa macchina, `C:\Program Files\Google\Chrome\Application\chrome.exe`, mentre Edge **non** sta nel percorso solito; sulle due immagini della CI — `ubuntu-latest` e `windows-latest` — ci sono Chrome ed Edge, dai README di `actions/runner-images`. 🔶 Sull'altra macchina serve Chrome, o `npx playwright install chromium`: si verifica là. ⚠️ **Richiamo del 2026-09-23 (R2-8):** col canale `chrome` il Chromium scaricato **non** vale — Playwright cerca Google Chrome stabile, e per un Chrome che manca dice `Run "npx playwright install chrome"` —: il ripiego è **`npx playwright install chrome`**, o Chrome installato a mano |
...
| 2 | il **contrasto** | `themes.css` | `contrast.test.ts` allargato: ogni `--color-text-*` su ogni `--color-bg-*` a 4,5:1 in ciascun tema; bordo forte, focus e segni a 3:1; le due esenzioni scritte accanto. ⚠️ **Richiamo del 2026-09-23 (R1-8, scelta A del proprietario):** per **famiglie** — ciascun testo sui fondi della sua famiglia a 4,5:1, i segni a 3:1 —, non ogni testo su ogni fondo; gli esenti sono i ruoli che il compito 1 del piano nomina, e una guardia vuole ogni ruolo in una famiglia o fra loro | (f) |
...
| `DockviewTheme`, i suoi campi — non sette: il comando è nella (c), richiamo del 2026-09-23 (R3-22) | `gui/node_modules/dockview-core/dist/cjs/dockview/theme.d.ts` |
...
|---|---|---|
| 1 | **la rilettura di questo disegno** | le sezioni sono approvate nel merito; da leggere come **aggiunte di chi scrive** sono le righe *«proposta»* della tabella dei controlli, le decisioni 23–27 e le precisioni di *«Cosa questo disegno ha misurato»*. Il consiglio: rileggerlo prima del piano, com'è stato per ogni disegno. ✅ **Scelto il 2026-09-23: A**, in chat all'apertura della sessione del piano — *«Sì, vai col piano»* — col consiglio del coordinatore di quella sessione, detto con la ragione: le aggiunte non cambiano nessuna decisione approvata, e si cambiano nel piano, che è testo. Il piano scrive **i consigli**: le dieci righe *«proposta»*, le decisioni 23–27 e le precisioni 4–11, così come sono |
...
| 22 | il browser dei test è il **Chrome installato**, canale `chrome` | c'è su questa macchina — `C:\Program Files\Google\Chrome\Application\chrome.exe` — e su tutte e due le immagini della CI, mentre Edge qui **non** sta nel percorso solito; il Chromium di Playwright andrebbe scaricato. Costo: 🔶 sull'altra macchina serve Chrome, o `npx playwright install chromium` — si verifica là. ⚠️ **Richiamo del 2026-09-23 (R2-8):** col canale `chrome` il ripiego è `npx playwright install chrome`, non `chromium` — la (f), *«dove c'è»* |
...
| **la CI** | i commit di questa sessione partono col push: la sessione dopo la legge **per prima**, coi due comandi di [`porta-di-qualita.md`](../../porta-di-qualita.md), il paragrafo *«Leggere la CI da terra»* — due job per corsa. ⚠️ **Richiamo del 2026-09-23:** qui stava *«del punto 3 della quarantunesima chiusura del piano della parte 2»*; i comandi sono gli stessi, la casa è cambiata col taglio 3 del ridimensionamento della lettura |
