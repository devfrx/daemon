...
✅ **RICHIAMO DEL 2026-09-23, quarta sessione — la (d) approvata.** La quarta sessione ha rifatto i controlli dell'apertura,
ha presentato la **(d)**, la cornice, e ha le risposte **19** — le miniature della Panoramica sono **schemi** — e **20** — la
striscia resta una pillola e si alza a 24 px dal fondo, perché gli angoli della finestra sono di Windows — con la
[tavola degli angoli](2026-09-22-design-system-tavole/angoli-finestra.html). ⛔ Si riprende dalla **(e)**, le voci
registrate: *«Prossimo passo, eseguibile»*, punto 4.
...
metà**: cominciato il 2026-09-23, venti risposte nella tabella *«Le risposte del proprietario, una per domanda»* — il
...
⛔ **Il prossimo passo è la sezione (e)**, le voci registrate che incrociano il design system. Si riprende **ESATTAMENTE** da
...
- ⛔ **il disegno a sezioni** — il prossimo passo, una sezione approvata per volta; gli approcci sono decisi, **A** alla
  risposta 14, la **(a)** è approvata alla 16, la **(b)** alla 17 e la **(c)** alla 18; la **(d)** alla 19 e alla 20: si riprende
  dalla **(e)**; le domande sono finite, e la
...
4. Gli **approcci** — ✅ **A**, risposta 14 — e il **disegno a sezioni**, una approvata per volta: ✅ (a) i token, risposte 15 e
   16; ✅ (b) il kit, 17; ✅ (c) il dock, 18; ✅ (d) la cornice, 19 e 20. ⛔ **Si riprende ESATTAMENTE dalla presentazione della
   (e)**: le voci della tabella *«Le voci registrate e non prese che toccano la GUI»* che incrociano il design system — M-3 e
   N-2 di E187, E228, N-2 di E235 — ciascuna riletta contro il codice di oggi prima di portarla, perché una voce può essere
   già chiusa da una sezione approvata (M-3, per esempio, e `BaseStatus` della risposta 17). Poi la **(f)**, le sonde che
   diventano test — contrasto, raggi, caratteri, icone, testo tagliato — col browser vero per le prove di disposizione, che è
   una **dipendenza nuova** e quindi del proprietario: `@vitest/browser-playwright` 4.1.11 accanto al nostro `vitest` 4.1.11,
   e `playwright` alla 1.63.0, da riverificare quel giorno. Se una sezione ha una domanda visiva, la tavola si genera nello
   scratchpad e si prova con le sonde **prima** di mostrarla, come `token.html` e `angoli-finestra.html`; e prima di credere a
   un verde si guardano i **conteggi** — `near`, le righe trovate — non il solo verdetto. I fatti della (d) com'erano prima
   di presentarla li rende `git show 36e44be:docs/superpowers/specs/2026-09-22-design-system-design.md`.
